...
# 从配置导入窗口名称（如果配置中有定义）
try:
    # 尝试从config模块导入WINDOW_NAME作为默认窗口名
    from config import WINDOW_NAME as DEFAULT_WINDOW_NAME
except ImportError:
...
try:
    from device.emulator_manager import EmulatorWindowFinder, HAS_WIN32
...
class KeyboardController:
    ...
    VK_CODES = {
        'w': 0x57, 'a': 0x41, 's': 0x53, 'd': 0x44,  # 方向键WASD
        'q': 0x51, 'e': 0x45, 'r': 0x52, 't': 0x54,  # 技能键QERT
        'f': 0x46, 'c': 0x43, 'u': 0x55, 'b': 0x42,  # 功能键FCUB
        '1': 0x31, '2': 0x32, '3': 0x33, '4': 0x34,  # 数字键1-4
        'space': 0x20,  # 空格键
    }
    
    # 扩展键标记集合
    # 这些键在发送消息时需要设置扩展标志位
    EXTENDED_KEYS = {'up', 'down', 'left', 'right', 'home', 'end', 'prior', 'next', 'insert', 'delete'}
    
    def __init__(self, window_name: str = None, hwnd: int = None):
        """
        初始化键盘控制器
        ...
        top_hwnd = 0
        if HAS_EMULATOR_FINDER:
            try:
                finder = EmulatorWindowFinder()
                # 尝试查找窗口（不检查分辨率，因为我们只需要发送消息）
                hwnd_found, title_found, rect_found, emu_type = finder.find_window(check_resolution=False)
                top_hwnd = hwnd_found
                self.window_name = title_found
                logger.info(f"通过 EmulatorWindowFinder 找到窗口: '{title_found}' (HWND: {top_hwnd})")
        ...
        # 记录初始化成功信息
        logger.info(f"KeyboardController 初始化完成: hwnd={self._hwnd}, 渲染子窗口={self._render_hwnd}")
    
    def _find_render_child_window(self, parent_hwnd):
        """
        查找渲染子窗口
        ...
        logger.debug(f"未找到渲染子窗口，找到的子窗口: {[(hwnd, cls) for hwnd, cls in child_windows]}")
        return None
    
    def _find_largest_visible_child(self, parent_hwnd):
        """
        查找最大的可见子窗口
        ...
        if largest_hwnd:
            ...
            logger.debug(f"选择最大可见子窗口: {class_name} (HWND: {largest_hwnd}, area: {largest_area})")
    ...
    def _send_key(self, key, is_press):
        ...
        try:
            ...
            if self._key_send_count < 10:
                logger.debug(f"PostMessage: key={key}, msg={msg_name}, hwnd={self._hwnd}")
            elif self._key_send_count % 100 == 0:
                logger.debug(f"PostMessage (第{self._key_send_count}次): key={key}")
        except Exception as e:
            logger.error(f"PostMessage 失败: key={key}, hwnd={self._hwnd}, msg={msg_name}, error={e}")
...
def tap(key, times=1, interval=None):
    ...
    for i in range(times):
        ...
        if i < times - 1:
            ...
            sleep_time = interval if interval is not None else random.uniform(0.03, 0.07)
            # 等待指定时间
