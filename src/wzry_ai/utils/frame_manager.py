...
class DummyStderr:
    ...
    # 定义需要过滤的关键词列表
    FILTER_KEYWORDS = [
        'sps_id', 'out of range', 'non-existing PPS',  # 参数集相关错误
        'no frame', 'decode_slice_header', 'referenced', # 帧解码相关错误
        'PPS', 'SPS', 'NAL', 'colocated'                # 其他技术关键词
    ]
...
class FrameManager:
    ...
    @classmethod
    def get_instance(cls, device_serial: str = None) -> 'FrameManager':
        """
        获取指定设备的帧管理器实例（单例模式）
    ...
    def get_latest_frame(self, timeout: float = 0.0, 
                        wait_new: bool = False) -> Optional[np.ndarray]:
        """
        获取最新帧图像
...
# ========== 便捷函数 ==========
def get_frame(device_serial: str = None, 
              timeout: float = 0.1) -> Optional[np.ndarray]:
    """
    快速获取一帧图像（便捷函数）
        ...
        try:
            ...
            for i in range(100):
                ...
                if frame is not None:
                    # 获取到帧，打印帧信息和FPS
                    logger.debug(f"帧 #{i}: {frame.shape}, FPS: {manager.get_stats()['avg_fps']:.1f}")
                else:
                    # 未获取到帧，打印无数据
