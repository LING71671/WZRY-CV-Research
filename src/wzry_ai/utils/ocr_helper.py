...
import os as _os
_nvidia_base = _os.path.join(_os.path.dirname(_os.__file__), '..', 'Lib', 'site-packages', 'nvidia')
...
try:
    ...
    _EasyOCR_cls = _easyocr_module.Reader
    ...
    def _do_init():
        ...
        try:
            ...
            if _OCR_BACKEND == 'paddle' and _PaddleOCR_cls is not None:
                ...
                import paddle
                logger.info(f"PaddlePaddle {paddle.__version__} (CUDA={paddle.is_compiled_with_cuda()}, GPU={paddle.device.cuda.device_count()})")
                ...
                _device = 'gpu' if paddle.is_compiled_with_cuda() and paddle.device.cuda.device_count() > 0 else 'cpu'
            ...
            # 备选：使用 EasyOCR
                ...
                logger.info(f"PyTorch {_torch.__version__} (CUDA={_torch.cuda.is_available()})")
                ...
                raise RuntimeError("没有可用的 OCR 引擎（PaddleOCR 和 EasyOCR 都未能导入）")
...
def recognize_text(frame: np.ndarray, region: Optional[Tuple[int, int, int, int]] = None) -> List[Tuple[str, float]]:
    """
    识别图像中的文字
        ...
        if _OCR_BACKEND == 'paddle':
            ...
            if result_list:
                for item in result_list:
                    # 新版格式是字典，包含 rec_texts 和 rec_scores
                    if hasattr(item, 'get'):
            ...
            # EasyOCR 使用 readtext 方法
            # 返回格式: [(bbox, text, confidence), ...]
            result = engine.readtext(rgb_frame)
            
            for item in result:
                if len(item) >= 3:
                    # item[0] = bbox, item[1] = text, item[2] = confidence
                    text = item[1]
                    confidence = item[2]
...
def find_text(frame: np.ndarray, keywords: List[str], region: Optional[Tuple[int, int, int, int]] = None) -> Optional[Tuple[str, float]]:
    """
    查找指定关键词，返回第一个匹配
...
def check_text_score(frame: np.ndarray, keywords: List[str], region: Optional[Tuple[int, int, int, int]] = None) -> float:
    """
    检查指定文字的综合置信度分数
    ...
    for text, conf in texts:
        for keyword in keywords:
            if keyword in text and conf > max_score:
                max_score = conf
                logger.debug(f"检测到文字: '{text}' 包含 '{keyword}' (置信度: {conf:.2f})")
