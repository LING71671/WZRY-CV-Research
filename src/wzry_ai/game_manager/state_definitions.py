...
# 尝试从配置模块导入模板置信度配置
try:
    from wzry_ai.config import TEMPLATE_CONFIDENCE_THRESHOLDS, DEFAULT_TEMPLATE_CONFIDENCE
except ImportError:
...
def get_confidence(template_name: str) -> float:
    ...
    """
    # 从配置字典中获取指定模板的配置
    config = TEMPLATE_CONFIDENCE_THRESHOLDS.get(template_name, DEFAULT_TEMPLATE_CONFIDENCE)
...
STATE_SIGNATURES = {
    ...
    GameState.LAUNCHER: StateSignature(
        ...
        exclude_templates=["game_lobby", "start_game", "battle", "ranking"],  # 排除大厅相关模板
    ...
    GameState.HALL: StateSignature(
        ...
        exclude_templates=["canyon_match", "ranking", "select_hero", "match_confirm"],  # 添加 match_confirm
        group="startup",
    ...
    GameState.BATTLE_5V5_SUB: StateSignature(
        ...
        exclude_templates=["ai_standard", "ai_quick", "start_match"],  # 排除下一级界面的模板
        group="battle_flow",
        description="5V5王者峡谷"
    ),
    ...
    GameState.BATTLE_AI_MODE: StateSignature(
        primary_templates=["ai_mode_choose"],  # 人机模式设置界面的标题（此界面唯一标识）
        exclude_templates=["wangzhe_canyon", "battle_mode", "start_match"],  # 排除其他界面
        group="battle_flow",
    ...
    GameState.HERO_SELECT: StateSignature(
        ...
        exclude_templates=["game_lobby",  # 排除大厅
                          "victory", "defeat", "continue",  # 排除结算
                          "match_statistics", "return_room", "Hero_Confirm",  # 排除赛后
                          "close1", "close2", "close3", "close4"],  # 排除弹窗
    ...
    GameState.LANE_SELECT: StateSignature(
        primary_templates=["lane_top", "lane_jungle", "lane_mid",  # 检测各分路按钮
                         "lane_adc", "lane_support", "all_lane"],
        exclude_templates=[],  # 无排除模板
        group="game_core",
        description="分路选择"
    ),
    ...
    GameState.IN_GAME: StateSignature(
        ...
        exclude_templates=["close1", "close2", "close3", "close4", "game_lobby", "select_hero", "wzry_icon", "victory", "victory1", "defeat"],  # 排除其他所有状态
