...
from typing import Tuple, List, Optional, Dict  # 导入类型提示工具
...
class PopupAction:
    ...
    confidence_threshold: float = None  # 置信度阈值（None则从config读取）
    ...
    def __post_init__(self):
        ...
        if self.confidence_threshold is None:  # 如果阈值未指定
            self.confidence_threshold = TEMPLATE_CONFIDENCE_THRESHOLDS.get(
                self.template, POPUP_DETECTION_THRESHOLD  # 从配置读取，不存在则使用默认值
            )
...
class PopupHandler:
    ...
    def handle_unknown_state(self, img_gray, current_depth: int = 0) -> Tuple[bool, int]:
        """
        处理未知状态 - 递归检测并处理弹窗
        ...
        if current_depth >= self.max_depth:  # 如果达到最大递归深度
            logger.warning(f"达到最大递归深度 {self.max_depth}，停止处理")  # 记录警告日志
            return False, current_depth  # 返回失败和当前深度
        
        # 检查点击冷却
        now = time.time()  # 获取当前时间
        if now - self.last_click_time < self.click_cooldown:  # 如果距离上次点击时间小于冷却时间
            time.sleep(self.click_cooldown - (now - self.last_click_time))  # 等待剩余冷却时间
        
        logger.info(f"未知状态处理 - 第 {current_depth + 1}/{self.max_depth} 层")  # 记录当前处理层数
        
        # 1. 尝试检测并处理弹窗
        popup_found, action_taken = self._try_handle_popup(img_gray)  # 调用弹窗检测处理方法
        ...
        # 2. 记录处理
        self.processed_popups.append({  # 将处理记录添加到列表
    ...
    def _try_handle_popup(self, img_gray) -> Tuple[bool, Optional[PopupAction]]:
        ...
        for action in self.POPUP_ACTIONS:  # 遍历所有弹窗动作配置
            ...
            if result.found:  # 如果检测到弹窗
                logger.info(f"检测到弹窗: {action.description} "
                      f"(置信度: {result.confidence:.2f})")  # 记录检测信息
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'max_depth_reached': max([p['depth'] for p in self.processed_popups]) if self.processed_popups else 0,  # 达到的最大深度
