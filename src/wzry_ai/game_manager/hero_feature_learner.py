...
class HeroFeatureLearner:
    ...
    TEMPLATE_FALLBACK_THRESHOLD = 0.40  # 模板匹配回退阈值，模板置信度在此范围内时启用深度学习
    ...
    def __init__(self, feature_db_path: str = None):
        ...
        if feature_db_path is None:
            ...
            feature_db_path = os.path.join(PROJECT_ROOT, 'data', 'hero_features.pkl')
        
        self.feature_db_path = feature_db_path  # 保存特征数据库文件路径
        self.features: dict[str, list[HeroFeature]] = {}  # 特征字典，键是英雄名，值是该英雄的特征列表
    ...
    def _init_feature_extractor(self):
        ...
        try:
            ...
            self.transform = transforms.Compose([
                ...
                transforms.Resize((224, 224)), # 调整图像大小为224x224（模型输入尺寸）
                transforms.ToTensor(),         # 转换为PyTorch张量
                transforms.Normalize(mean=[0.485, 0.456, 0.406],   # ImageNet均值标准化
                                   std=[0.229, 0.224, 0.225])       # ImageNet标准差标准化
            ])
    ...
    def match(self, img: np.ndarray, hero_list: list[str]) -> tuple[Optional[str], float]:
        """
        将输入图像与英雄列表进行匹配
        ...
        for hero_name in hero_list:
            ...
            for hero_feature in self.features[hero_name]:
                # 计算余弦相似度
                sim = self._cosine_similarity(query_feature, hero_feature.feature_vector)
    ...
    def learn(self, hero_name: str, img: np.ndarray, confidence: float):
        ...
        # 记录学习信息
        logger.info(f"学习新特征: {hero_name} (当前共{len(self.features[hero_name])}个特征)")
