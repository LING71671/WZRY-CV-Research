...
class SkillContext:
    ...
    # ========== 基础状态字段 ==========
    self_health: Optional[int] = None           # 自身血量值，整数类型，None表示处于附身状态无法获取自身血量
    team_health: List[Any] = field(default_factory=list)    # 队友血量列表，存储每个队友的血量信息
    enemy_health: List[Any] = field(default_factory=list)   # 敌人血量列表，存储每个敌人的血量信息
    team_positions: List[float] = field(default_factory=list)   # 队友距离列表，存储每个队友与自身的距离（像素）
    enemy_positions: List[float] = field(default_factory=list)  # 敌人距离列表，存储每个敌人与自身的距离（像素）
    
    # ========== 附身状态字段（新版：优先使用is_attached字段判断） ==========
    is_attached: bool = False                   # 是否处于附身状态，True表示已附身到队友身上，由model2_detector检测提供
    yao_state: str = "unknown"                  # 瑶的详细状态字符串：normal（正常）/attached（附身）/deer（鹿灵）/unknown（未知）
    attached_hero_name: Optional[str] = None    # 附身的英雄名称，字符串类型，None表示未附身或未知
    attach_start_time: Optional[float] = None   # 附身开始时间，时间戳（秒），用于计算附身持续时间
    
    # ========== 扩展信息字段 ==========
    priority_teammates: List[str] = field(default_factory=list)  # 优先队友列表，存储需要优先保护或附身的队友英雄名称
    teamfight_detected: bool = False            # 是否检测到团战，True表示当前处于团战状态
    escape_needed: bool = False                 # 是否需要逃跑，True表示当前处于需要逃跑的紧急状态
    
    # ========== 时间戳字段 ==========
    timestamp: float = field(default_factory=time.time)  # 上下文创建时间戳，默认为当前时间
    ...
    def is_self_low_hp(self, threshold: int = 50) -> bool:
        ...
        """
        if self.is_attached or self.self_health is None:  # 检查是否处于附身状态或血量未知
    ...
    def get_attach_duration(self) -> Optional[float]:
        ...
        """
        if not self.is_attached or self.attach_start_time is None:  # 检查是否处于附身状态且开始时间有效
            return None  # 未附身或时间未知时返回None
        return time.time() - self.attach_start_time  # 计算当前时间与附身开始时间的差值
        
    @classmethod
    def from_health_info(cls, health_info: Dict, priority_teammates: List[str] = None) -> 'SkillContext':
        """
        ...
        context = cls(
            ...
            team_health=health_info.get('team_health', []),  # 从字典获取队友血量，默认为空列表
            enemy_health=health_info.get('enemy_health', []),  # 从字典获取敌人血量，默认为空列表
            team_positions=health_info.get('team_positions', []),  # 从字典获取队友位置，默认为空列表
            enemy_positions=health_info.get('enemy_positions', []),  # 从字典获取敌人位置，默认为空列表
            is_attached=health_info.get('is_attached', False),  # 从字典获取附身状态，默认为False，由model2_detector提供
            yao_state=health_info.get('yao_state', 'unknown'),  # 从字典获取瑶的状态，默认为unknown
            priority_teammates=priority_teammates or [],  # 设置优先队友列表，None时默认为空列表
        )
