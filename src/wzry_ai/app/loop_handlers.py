...
class LoopHandlers:
    ...
    def _start_detection_threads(self):
        ...
        # 启动模态1检测线程（小地图检测）
        model1_thread = threading.Thread(
            target=self._run_model1_detection,
            daemon=True,
            name="Model1Detection"
        )
        model1_thread.start()
        self.services.thread_supervisor.register_thread("model1_detection", model1_thread)
        logger.info("✓ Model1 检测线程已启动")
        
        # 启动模态2检测线程（全屏血条检测）
        model2_thread = threading.Thread(
            target=self._run_model2_detection,
            daemon=True,
            name="Model2Detection"
        )
        model2_thread.start()
        self.services.thread_supervisor.register_thread("model2_detection", model2_thread)
        logger.info("✓ Model2 检测线程已启动")
        
        # 启动游戏状态检测线程
        state_thread = threading.Thread(
            target=self._run_state_detection,
            daemon=True,
            name="StateDetection"
        )
        state_thread.start()
        self.services.thread_supervisor.register_thread("state_detection", state_thread)
    ...
    def _run_model1_detection(self):
        ...
        try:
            from wzry_ai.detection.model1_detector import Model1Detector
            detector = Model1Detector()
        ...
        while True:
            try:
                ...
                # 执行检测
                result = detector.detect(minimap_frame)
    ...
    def _run_model2_detection(self):
        ...
        try:
            from wzry_ai.detection.model2_detector import Model2Detector
            detector = Model2Detector()
        ...
        while True:
            try:
                ...
                # 执行检测
                result = detector.detect(full_frame)
    ...
    def _run_state_detection(self):
        ...
        while True:
            try:
                ...
                # 执行状态检测
                result = self.services.state_detector.detect(frame)
                
                # 记录状态变化
                if result.action_taken:
                    logger.info(f"游戏状态: {result.state} -> 执行动作: {result.action}")
    ...
    def _update_debug_windows(self, frame):
        ...
        # 添加FPS文字
        cv2.putText(small_frame, f"FPS: {self._current_fps:.1f}", 
