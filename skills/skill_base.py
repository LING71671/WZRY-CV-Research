...
class SkillConfig:
    ...
    trigger_conditions: List[TriggerCondition] = None  # 触发条件列表，默认为None
    trigger_params: Dict[str, Any] = None  # 触发条件参数，字典类型，默认为None
...
class SkillBase(ABC):
    ...
    def get_remaining_cooldown(self) -> float:
        ...
        """
        if self.config.cooldown_type == CooldownType.NO_COOLDOWN:  # 检查是否为无冷却类型
            return 0  # 无冷却，直接返回0
    ...
    def _evaluate_condition(self, condition: TriggerCondition, context) -> bool:
        ...
        if condition == TriggerCondition.HAS_ENEMY:
            ...
            return closest is not None and closest <= self.config.range  # 检查是否在技能范围内
        ...
        # ========== 队友相关条件评估 ==========
            ...
            return context.has_priority_teammate_in_range(self.config.range)  # 检查优先队友是否在范围内
        ...
        # ========== 战斗状态条件评估 ==========
            ...
            return context.enemy_count >= threshold and context.has_teammate  # 检查是否团战（敌人多且有队友）
...
class SkillRegistry:
    ...
    def create_skill(cls, config: SkillConfig) -> SkillBase:
        ...
        if skill_class is None:  # 检查技能类是否存在
            raise ValueError(f"未注册的技能类型: {config.skill_type}")  # 未注册则抛出异常
