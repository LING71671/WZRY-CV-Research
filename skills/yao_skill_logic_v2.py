...
YAO_SKILL_CONFIG = {
    "Q": {  # 一技能配置 - 伤害/控制技能
        ...
        "range": 500,              # 技能作用范围（像素）
        "trigger_conditions": ["has_enemy", "enemy_in_range"],  # 触发条件：有敌人在范围内
        "priority": 3,             # 技能优先级
    ...
    "R": {  # 大招配置 - 附身技能
        ...
        "range": 350,              # 技能作用范围（像素）
        "trigger_conditions": ["has_teammate", "teammate_in_range"],  # 触发条件：有队友在范围内
    ...
    "confluence": {  # 召唤师技能-汇流为兵-辅助配置
        ...
        "trigger_conditions": ["self_low_hp", "teammate_low_hp"],  # 触发条件：自身或队友低血量
    ...
    "recover": {  # 召唤师技能-恢复配置
        ...
        "range": 0,                # 技能作用范围（0表示以自身为中心）
        "trigger_conditions": ["self_low_hp", "peace_state"],  # 触发条件：自身低血量且和平状态
...
# 从基类导入需要的常量和基类
from skills.hero_skill_logic_base import (
    HeroSkillLogicBase,
    KEY_CAST_1, KEY_CAST_2, KEY_CAST_ULT, KEY_SKILL_4,
    KEY_LEVEL_ULT, KEY_LEVEL_1, KEY_LEVEL_2, KEY_BUY_ITEM,
    KEY_ATTACK, KEY_HEAL, KEY_RECOVER,
    HEAD_TO_FEET_OFFSET_X, HEAD_TO_FEET_OFFSET_Y,
    REDEMPTION_RANGE_X, REDEMPTION_RANGE_Y_UP, REDEMPTION_RANGE_Y_DOWN,
    HEAL_RANGE_X, HEAL_RANGE_Y_UP, HEAL_RANGE_Y_DOWN,
    HEAD_TO_FEET_OFFSET_GENERIC,
)

# 从基类导入日志记录器
from skills.hero_skill_logic_base import logger

# 从统一配置导入召唤师技能设置
from config import ACTIVE_SUMMONER_F
from config import MINIMAP_SCALE_X, MINIMAP_SCALE_Y

from utils.frame_manager import get_frame
    ...
    def detect(self, frame, yolo_self_detected=True):
        ...
        try:
            ...
            for state, ref_color in COLOR_REF.items():
                # 计算欧几里得距离
                distance = sqrt(sum((avg_color[i] - ref_color[i])**2 for i in range(3)))
...
E_RANGE_Y_DOWN_ATTACHED = 595   # E技能Y轴下半轴长度（附身状态）：496 * 1.20 = 595.2 ≈ 595
...
class YaoSkillLogic(HeroSkillLogicBase):
    ...
    def _filter_self_health(self, raw_health, current_time, raw_pos=None):
        ...
        if last_valid is None:
            ...
            if pending is None:
                ...
                self._log(f"[血量确认] 首次检测到自身血量 {raw_health}%，开始确认(需{required_frames}帧)...")
                return None
            else:
                if abs(raw_health - pending) <= CONFIRM_THRESHOLD:
                    ...
                    if confirm_count >= required_frames:
                        ...
                        self._log(f"[血量确认] 自身血量 {pending}% 已确认（连续{confirm_count}帧）")
                        self._health_history['last_valid_self'] = pending
                        self._health_history['pending_health'] = None
                        self._health_history['confirm_count'] = 0
                        return pending
                    else:
                        self._log(f"[血量确认] 自身血量 {pending}% 确认中 ({confirm_count}/{required_frames})")
                        return None
                else:
                    self._log(f"[血量确认] 血量波动 {pending}% -> {raw_health}%，重置确认")
                    self._health_history['pending_health'] = raw_health
                    self._health_history['confirm_count'] = 1
                    return None
        ...
        if change > MUTATION_THRESHOLD:
            ...
            if not is_rising and raw_health < 10:
                self._log(f"[血量过滤] 下降突变到极低血量 {last_valid:.0f}% -> {raw_health:.0f}%，视为干扰")
                return last_valid
            
            if len(history) >= 2:
                prev_health = history[-2][0] if history[-2][0] is not None else last_valid
                prev_change = abs(raw_health - prev_health)
                
                if is_rising and raw_health > 80:
                    # 上升突变到高血量（如1%->93%），更可能是恢复正常
                    self._log(f"[血量过滤] 上升突变 {last_valid:.0f}% -> {raw_health:.0f}%，可能是恢复，接受新值")
                    self._health_history['last_valid_self'] = raw_health
                    return raw_health
                elif prev_change > MUTATION_THRESHOLD:
                    # 连续两帧变化都超过阈值，视为干扰
                    self._log(f"[血量过滤] 自身血量突变 {last_valid:.0f}% -> {raw_health:.0f}%，视为干扰")
                    return last_valid
                else:
                    # 前一帧稳定，当前帧突变，可能是真实变化
                    self._log(f"[血量过滤] 血量变化 {last_valid:.0f}% -> {raw_health:.0f}%（前一帧稳定），接受新值")
            else:
                # 历史记录不足，但变化超过阈值，谨慎处理
                if not is_rising and raw_health < 20:
                    # 下降突变到较低血量，可能是错误
                    self._log(f"[血量过滤] 下降突变 {last_valid:.0f}% -> {raw_health:.0f}%（历史不足），视为干扰")
                    return last_valid
                else:
                    # 其他情况接受变化
                    self._log(f"[血量过滤] 血量变化 {last_valid:.0f}% -> {raw_health:.0f}%（历史不足但合理），接受新值")
    ...
    def _filter_team_health(self, raw_team_health, current_time):
        ...
        for idx, teammate in enumerate(raw_team_health):
            if not isinstance(teammate, dict) or 'health' not in teammate or 'pos' not in teammate:
                continue
            ...
            for known_idx, known_pos in team_positions.items():
                if known_pos is not None:
                    dist = sqrt((current_pos[0] - known_pos[0])**2 + (current_pos[1] - known_pos[1])**2)
        ...
        for matched_idx, teammate, current_pos, raw_hp in matched_teammates:
            ...
            if last_pos is not None:
                dist = sqrt((current_pos[0] - last_pos[0])**2 + (current_pos[1] - last_pos[1])**2)
                if dist > POSITION_THRESHOLD:
                    self._log(f"[血量过滤] 队友{matched_idx}位置突变 ({last_pos[0]:.0f},{last_pos[1]:.0f}) -> ({current_pos[0]:.0f},{current_pos[1]:.0f})，可能是不同目标")
                    # 重置该索引的历史记录
            ...
            if last_valid is None or change > 20:
                ...
                if pending_info is None:
                    ...
                    team_confirm[matched_idx] = 1
                    if is_suspicious_drop:
                        self._log(f"[血量警告] 队友{matched_idx}血量大幅下降 {last_valid}%->{raw_hp}%，需{required_frames}帧确认")
                    else:
                        self._log(f"[血量确认] 队友{matched_idx}血量 {raw_hp}% 开始确认(需{required_frames}帧)...")
                    if last_valid is not None:
                        teammate = dict(teammate)
                        teammate['health'] = last_valid
                        filtered.append(teammate)
                    ...
                    pos_dist = sqrt((current_pos[0] - pending_pos[0])**2 + (current_pos[1] - pending_pos[1])**2)
                    ...
                    if hp_consistent and pos_consistent:
                        ...
                        if team_confirm[matched_idx] >= required_frames:
                            ...
                            if is_suspicious_drop and pending_hp < LOW_HP_THRESHOLD:
                                self._log(f"[血量警告] 队友{matched_idx}确认低血量 {pending_hp}%，可能存在检测错误")
                            self._log(f"[血量确认] 队友{matched_idx}血量 {pending_hp}% 已确认")
                            ...
                            self._log(f"[血量确认] 队友{matched_idx}血量 {pending_hp}% 确认中 ({team_confirm[matched_idx]}/{required_frames})")
                            if last_valid is not None:
                                teammate = dict(teammate)
                                teammate['health'] = last_valid
                                filtered.append(teammate)
                        ...
                        # 不一致，重置
                        self._log(f"[血量确认] 队友{matched_idx}波动 血量:{pending_hp}%->{raw_hp}% 位置变化:{pos_dist:.0f}px，重置确认")
                        team_pending[matched_idx] = (raw_hp, current_pos)
                        team_confirm[matched_idx] = 1
                        if last_valid is not None:
                            teammate = dict(teammate)
            ...
            if last_valid is not None:
                ...
                if change > 40:
                    if len(history) >= 2:
                        trend = sum([h for h, t in history[-3:] if h is not None]) / len([h for h, t in history[-3:] if h is not None])
                        if abs(raw_hp - trend) > 30:
                            self._log(f"[血量过滤] 队友{matched_idx}血量突变 {last_valid:.0f}% -> {raw_hp:.0f}%，平滑处理")
                            teammate = dict(teammate)
    ...
    def check_and_use_skills(self):
        ...
        skill_policy = self.health_info.get('skill_policy', 'aggressive') if self.health_info else 'aggressive'
        ...
        allow_offensive = (skill_policy == 'aggressive') or (skill_policy == 'conservative' and has_enemy)
        
        # --- 血量防干扰过滤 ---
        # 自身血量过滤：连续性检查 + 突变检测 + 检测框稳定性
        raw_self_pos = self.health_info.get('self_pos') if self.health_info else None
        self_health = self._filter_self_health(raw_self_health, current_time, raw_self_pos)
        ...
        if frame is not None:
            ...
            self.current_yao_state = self.state_detector.detect(frame, yolo_self_detected)
        else:
            # 备用方案：基于血量数据判断
        ...
        if is_attached or raw_self_health is None:
            ...
            # 目标锁定：优先保持已锁定的队友
            if self.locked_target is not None and self.lock_frame_count < self.TARGET_LOCK_FRAMES:
                # 查找锁定的目标是否仍在视野中
            ...
            if closest_team is None and self_feet_pos and team_health:
                ...
                for t in team_health:
                    if isinstance(t, dict) and 'pos' in t:
                        ...
                        td = sqrt((self_feet_pos[0]-team_feet_pos[0])**2 + 
            ...
            if closest_team is None and minimap_data:
                ...
                if m1_self and m1_teammates:
                    ...
                    for t in m1_teammates:
                        dx_screen, dy_screen = self._minimap_distance_to_screen(m1_self, t)
                    ...
                    if m1_best_dx is not None:
                        ...
                        closest_team = {'pos': None, 'health': None, '_from_minimap': True}
                        self._log(f"[R-小地图] 使用小地图坐标判断R技能 | 距离={m1_min_dist:.0f}px")
            
            # 如果找到目标，计算范围
            if closest_team is not None and closest_team_dx is None and self_feet_pos is not None:
                # 重新计算锁定目标的距离
            ...
            if closest_team_dx is not None and closest_team_raw_dy is not None:
                # 根据队友方向选择Y轴范围
                if closest_team_raw_dy < 0:  # 队友在上方
                    r_ry = R_RANGE_Y_UP
                else:  # 敌人在下方
                    r_ry = R_RANGE_Y_DOWN
                r_ellipse_val = (closest_team_dx / R_RANGE_X)**2 + (closest_team_dy / r_ry)**2
            ...
            if can_ult and not is_in_protect:
                self._log(f"[R] 释放 | 范围:椭圆{r_ellipse_val:.2f} | dx={closest_team_dx}, dy={closest_team_dy} | 目标锁定:{self.lock_frame_count}帧")
                ...
                # 保护期内但条件满足，记录日志
                self._log(f"[R] 保护期内无法附身 | 剩余{self.ULT_PROTECT_DURATION - protect_duration:.1f}秒")

        # --- 2. Q/E技能 ---
        # 策略检查：aggressive 始终允许，conservative 有敌人时允许（被动反击）
        
        # 诊断日志低频输出控制
        should_log_diag = (int(current_time) % 5 == 0) and (current_time - getattr(self, '_last_diag_log_time', 0) > 1)
        if should_log_diag:
        ...
        if has_enemy and self_feet_pos and allow_offensive:
            ...
            for e in enemy_health:
                if isinstance(e, dict) and 'pos' in e:
                    pos = e['pos']
                    enemy_feet_pos = self._convert_to_feet_pos(pos, use_generic_offset=True)
                    d = sqrt((self_feet_pos[0]-enemy_feet_pos[0])**2 + 
            ...
            if is_deer:
                q_in_range = True
            elif closest_dy is not None and raw_dy is not None:
                ...
                ellipse_val = (closest_dx / q_range_x)**2 + (closest_dy / q_range_y)**2
            ...
            # E技能
            if not is_deer and closest_dx is not None and raw_dy is not None:
                # 根据方向和附身状态选择范围（清晰的if-elif结构）
                if is_attached:
            ...
            # 调试日志：普攻条件（低频）
            if should_log_diag:
                self._log(f"[技能诊断-普攻] 未附身={not is_attached}, 未移动={not self.is_moving}")
            
            # 普攻（未附身时，添加范围判断）
            if not is_attached and closest_dx is not None and raw_dy is not None:
                # 使用E技能数值（临时，后续可独立配置）
                if raw_dy < 0:  # 敌人在上方
                    attack_ry = 333
                else:  # 敌人在下方
                    attack_ry = 522
                attack_ellipse_val = (closest_dx / 630)**2 + (closest_dy / attack_ry)**2
            ...
            if m1_self and m1_enemies:
                ...
                if m1_closest_dx is not None:
                    ...
                    if is_deer:
                        ...
                        q_range_x = Q_RANGE_X_ATTACHED if is_attached else Q_RANGE_X_NORMAL
                        ellipse_val = (m1_closest_dx / q_range_x)**2 + (m1_closest_dy / q_range_y)**2
                        q_in_range = (ellipse_val < 1)
                    
                    q_ready = (current_time - self.last_q_time > 4.0)
                    
                    if q_in_range and q_ready:
                        self._log(f"[Q-小地图] 释放（小地图补充） | 距离={m1_min_dist:.0f}px")
                        self.tap_skill(KEY_CAST_1, "一技能")
                        self.last_q_time = current_time
                        if is_deer:
                            self.last_e_time = 0
                    ...
                    if not is_deer:
                        if is_attached:
                            ...
                            if m1_raw_dy < 0:
                                ...
                                e_ry = E_RANGE_Y_DOWN
                        e_ellipse_val = (m1_closest_dx / e_rx)**2 + (m1_closest_dy / e_ry)**2
                        e_in_range = (e_ellipse_val < 1)
                    else:
                        e_in_range = False
                    ...
                    if e_in_range and e_ready:
                        self._log(f"[E-小地图] 释放（小地图补充） | 距离={m1_min_dist:.0f}px")
                        self.tap_skill(KEY_CAST_2, "二技能")
                        self.last_e_time = current_time
        ...
        if ACTIVE_SUMMONER_F == "confluence":
            ...
            if is_stable and confluence_cooldown_ok:
                ...
                if need_confluence:
                    self._log(f"[F] 汇流为兵-辅助 | 受益人数:{low_hp_count} (范围内队友:{len(in_range_team_health)})")
                    ...
                    for skill_name, (last_time, cd) in skill_cds.items():
                        ...
                        if remaining > 0:
                            ...
                            self._log(f"  [汇流] {skill_name} 剩余CD {remaining:.1f}s -> {remaining*0.5:.1f}s")
            ...
            if is_stable and (current_time - self.last_heal_time > 5):
                ...
                if need_heal:
                    self._log(f"[F] 治疗术 | 受益人数:{low_hp_count} (范围内队友:{len(in_range_team_health)})")
        ...
        if is_stable and t_ready and self_feet_pos and team_health:
            for idx, t in enumerate(team_health):
                if isinstance(t, dict) and 'pos' in t and 'health' in t:
                    ...
                    t_ellipse_val = (t_dx / REDEMPTION_RANGE_X)**2 + (t_dy / t_ry)**2
