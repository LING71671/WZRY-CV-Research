...
MINGSHIYIN_SKILL_CONFIG = {
    ...
    "R": {  # 大招配置 - 团队增益/治疗
        ...
        "range": 500,              # 技能作用范围（像素）
        "trigger_conditions": ["in_teamfight", "teammate_low_hp"],  # 触发条件：团战且队友低血量
        "trigger_params": {"hp_threshold": 40, "enemy_count": 2},   # 触发参数：血量40%，敌人数量2
        "priority": 1,             # 技能优先级
        "can_use_when_attached": True,   # 附身状态可用
        "can_use_when_detached": True,   # 非附身状态可用
    },
...
class MingshiyinSkillLogic(HeroSkillLogicBase):
    ...
    def check_and_use_skills(self):
        ...
        allow_offensive = (skill_policy == 'aggressive') or (skill_policy == 'conservative' and has_enemy_for_policy)
        ...
        if current_time - self.last_q_time > 8:
                ...
                for t in team_health:
                    if isinstance(t, dict) and 'pos' in t and 'health' in t:
                        ...
                        if h < min_team_hp:
                            ...
                            # 计算与队友的距离
                            d = sqrt((self_feet_pos[0]-team_feet_pos[0])**2 +
            ...
            if q_target is None and has_enemy and self_feet_pos:
                for e in enemy_health:
                    if isinstance(e, dict) and 'pos' in e:
                        epos = e['pos']
                        # 将敌人头顶坐标转换为脚下坐标
                        enemy_feet_pos = self._convert_to_feet_pos(epos, use_generic_offset=True)
                        # 计算与敌人的距离
            ...
            if q_target is not None and self_feet_pos:
                tpos = q_target['pos']
                target_feet_pos = self._convert_to_feet_pos(tpos, use_generic_offset=True)
                # 计算与目标的水平和垂直距离
                ...
                if q_in_range:
                    if q_target_type == 'team':
                        self._log(f"[Q] 链接队友 | 血量:{min_team_hp}% | 距离:{min_dist:.0f}")
        ...
        if has_enemy and current_time - self.last_e_time > 6 and self_feet_pos and allow_offensive:
            # 寻找最近的敌人
            ...
            if closest_enemy is not None:
                ...
                if e_in_range:
                    ...
                    self._log(f"[E] 控制 | 敌人数量:{enemy_count} | 距离:{min_e_dist:.0f}")
        ...
        if has_enemy and self_health is not None and self_health > 50 and allow_offensive:
            self.basic_attack()
        ...
        if ACTIVE_SUMMONER_F == "confluence":
            ...
            if need_confluence and current_time - self.last_confluence_time > 90 and is_stable:
                ...
                skill_cds = {
                    ...
                    'R': (self.last_ult_time, 60.0),     # 大招（团队增益），代码内检查CD=60秒
                }
