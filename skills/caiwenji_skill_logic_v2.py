...
class CaiwenjiSkillLogic(HeroSkillLogicBase):
    ...
    def check_and_use_skills(self):
        ...
        if self_feet_pos and enemy_health and not team_health:
            ...
            for e in enemy_health:
                if isinstance(e, dict) and 'pos' in e:
                    # 将敌人头顶坐标转换为脚下坐标
                    enemy_feet_pos = self._convert_to_feet_pos(e['pos'], use_generic_offset=True)
                    # 计算与敌人的欧几里得距离
        ...
        if has_enemy and self_feet_pos and allow_offensive:
            ...
            # Q技能判断和释放
            # 使用椭圆范围判断敌人在不在Q技能范围内
            if closest_dy is not None and raw_dy is not None:
            ...
            # E技能判断和释放
            if closest_dx is not None and raw_dy is not None:
                if raw_dy < 0:  # 敌人在上方，使用上方范围
                    e_ry = E_RANGE_Y_UP
                else:  # 敌人在下方，使用下方范围
                    e_ry = E_RANGE_Y_DOWN
        ...
        if ACTIVE_SUMMONER_F == "confluence":
            ...
            if is_stable and confluence_cooldown_ok:
                ...
                if need_confluence:
                    self._log(f"[F] 汇流为兵-辅助 | 受益人数:{low_hp_count} (范围内队友:{in_range_team_count})")
                    ...
                    skill_cds = {
                        'Q': (self.last_q_time, 4.0),       # Q技能（思无邪），代码内检查CD=4秒
                        'E': (self.last_e_time, 5.0),        # E技能（忘忧曲），代码内检查CD=5秒
                        'R': (self.last_ult_time, 5.0),      # 大招（团队回血），代码内检查CD=5秒
                    }
            ...
            if is_stable and (current_time - self.last_heal_time > 5):
                ...
                if need_heal:
                    self._log(f"[F] 治疗术 | 受益人数:{low_hp_count} (范围内队友:{in_range_team_count})")
