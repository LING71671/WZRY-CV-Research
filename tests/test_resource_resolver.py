...
class TestRuntimePathResolver:
    ...
    def test_find_first_existing(self):
        ...
        result = resolver.find_first_existing(
            repo_root / "nonexistent1",
            repo_root / "src",
            repo_root / "nonexistent2"
        )
...
class TestPathNormalization:
    ...
    def test_normalize_parts_with_none(self):
        """测试处理None值"""
        resolver = RuntimePathResolver()
        parts = resolver._normalize_parts([None, "test", "", "file.txt"])
        assert parts == ("test", "file.txt")
    ...
    def test_join_with_empty_parts(self):
        ...
        resolver = RuntimePathResolver()
        root = Path("/test/root")
        result = resolver._join(root, None, "", "file.txt")
