...
def _get_adb_path():
    ...
    if _ADB_PATH is None:
        try:
            from device.emulator_manager import get_adb_path
...
try:
    # 尝试导入模拟器管理器
    from device.emulator_manager import EmulatorPortFinder, MuMuConfigManager
    # 标记模拟器管理器可用
...
class ADBTool:
    ...
    def __init__(self, device_serial=None, auto_detect: bool = True, adb_path: str = None):
        """
        初始化ADB工具
    ...
    def _init_persistent_shell(self):
        ...
        self._persistent_shell = subprocess.Popen(
            [self.adb_path, "-s", self.device_serial.serial, "shell"],
    ...
    def _shell_cmd(self, cmd: str):
        ...
        try:
            self._persistent_shell.stdin.write(f"{cmd}\n".encode())
            self._persistent_shell.stdin.flush()
    ...
    def _shell(self, cmd: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            return True, output if output else ''
                ...
                RuntimeError, OSError, adbutils.AdbError) as e:
    ...
    def _ensure_connected(self) -> bool:
        ...
        try:
            ...
            # 执行连接命令
            subprocess.run(cmd, shell=True, capture_output=True, timeout=5, startupinfo=startupinfo)
            self._connected = True
            return True
    ...
    def _run_adb(self, command: str, timeout: int = 10, binary: bool = False):
        ...
        try:
            ...
            # 执行命令
            result = subprocess.run(full_cmd, shell=True, capture_output=True, timeout=timeout, startupinfo=startupinfo)
    ...
    def swipe(self, x1: int, y1: int, x2: int, y2: int, duration: int = 300) -> bool:
        ...
        # 通过持久 shell 滑动
        self._shell_cmd(f"input swipe {int(x1)} {int(y1)} {int(x2)} {int(y2)} {int(duration)}")
        return True
    ...
    def screenshot(self, save_path: str = None, use_scrcpy: bool = True) -> Optional[bytes]:
        """
        截图
...
def tap(x: int, y: int, device_serial: str = None, delay: float = 0.1) -> bool:
    """
    快速点击（便捷函数）
...
def swipe(x1: int, y1: int, x2: int, y2: int, device_serial: str = None, duration: int = 300) -> bool:
    """
    快速滑动（便捷函数）
