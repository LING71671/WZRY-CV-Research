...
try:
    from config.emulator import EMULATOR_ADB_PATHS, SCAN_DRIVES
except ImportError:
    # 导入失败，使用默认配置
    ...
    def _find_adb_from_mumu_process(cls) -> Optional[str]:
                ...
                try:
                    if proc.info['name'] in MUMU_PROCESS_NAMES:
                        ...
                        candidates.extend([
                            ...
                            os.path.join(os.path.dirname(exe_dir), 'EmulatorShell', 'adb.exe'),
                        ])
                        
                        for adb_path in candidates:
                            if os.path.isfile(adb_path):
                                logger.info(f"从 MuMu 进程 '{proc.info['name']}' 推导出 ADB: {adb_path}")
                                return adb_path
        ...
        # 如果 psutil 不可用，尝试使用 ctypes/win32process 获取进程路径
            ...
            try:
                ...
                if kernel32.EnumProcesses(process_ids, ctypes.sizeof(process_ids), ctypes.byref(cb_needed)):
                    ...
                    for i in range(num_processes):
                        ...
                        try:
                            ...
                            if h_process:
                                ...
                                if psapi.EnumProcessModules(h_process, ctypes.byref(h_module), 
                                                            ctypes.sizeof(h_module), 
                                                            ctypes.byref(cb_needed_module)):
                                    module_name = ctypes.create_unicode_buffer(260)
                                    if psapi.GetModuleBaseNameW(h_process, h_module, module_name, 260):
                                        proc_name = module_name.value
                                        if proc_name in MUMU_PROCESS_NAMES:
                                            # 获取进程完整路径
                                            full_path_buf = ctypes.create_unicode_buffer(260)
                                            if psapi.GetModuleFileNameExW(h_process, h_module, full_path_buf, 260):
                                                ...
                                                for adb_path in candidates:
                                                    if os.path.isfile(adb_path):
                                                        logger.info(f"从 MuMu 进程 '{proc_name}' 推导出 ADB: {adb_path}")
    ...
    def _find_system_adb(cls) -> str:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                output = result.stdout.decode('utf-8', errors='ignore') or result.stdout.decode('gbk', errors='ignore')
                # 按行分割输出
                ...
                try:
                    for root, dirs, files in os.walk(base_dir):
                        ...
                        dirs[:] = [d for d in dirs if d.lower() not in 
                                  ['windows', 'temp', 'tmp', 'cache', '$recycle.bin']]
...
def get_adb_path(emulator_type: str = None, auto_save: bool = True) -> str:
    ...
    功能说明：
    - 优先从 mumu_config.json 缓存读取 ADB 路径
    - 缓存无效时，按优先级查找 ADB 路径
    - 找到新路径后自动保存到缓存
    """
    # 声明使用全局变量
...
# 尝试从配置模块导入端口和型号配置
try:
    from config.emulator import EMULATOR_PORTS, EMULATOR_MODELS
except ImportError:
    # Fallback: 使用默认配置
    EMULATOR_PORTS = {
...
class EmulatorPortFinder:
    ...
    def find_port(self, prefer_cached: bool = True, emulator_type: str = None) -> Tuple[Optional[int], Optional[str]]:
        """
        查找模拟器端口
        ...
        if prefer_cached:
            ...
            if cached_port and self._test_port(cached_port):
                logger.info(f"使用缓存端口: {cached_port} ({cached_type or '未知类型'})")
    ...
    def _test_port(self, port: int) -> bool:
        ...
        try:
            ...
            test_output = test_result.stdout.decode('utf-8', errors='ignore') if test_result.stdout else ''
    ...
    def _detect_emulator_type(self, port: int) -> Optional[str]:
        ...
        try:
            ...
            manufacturer = result.stdout.decode('utf-8', errors='ignore').strip().lower()
    ...
    def _auto_learn_device_model(self, port: int, model: str, manufacturer: str) -> Optional[str]:
        """
        自动学习并保存未知设备型号
    ...
    def _save_device_models(self):
        ...
        try:
            ...
            config_file = os.path.join(PROJECT_ROOT, 'data', 'emulator_models.json')
...
# 尝试从配置模块导入窗口模式和分辨率配置
try:
    from config.emulator import (
        WINDOW_PATTERNS, EXPECTED_WIDTH, EXPECTED_HEIGHT, BORDER_WIDTH, TITLE_HEIGHT,
        MUMU_PROCESS_NAMES, MUMU_CLASS_NAMES, EXCLUDE_TITLE_KEYWORDS
    )
except ImportError:
    # Fallback: 使用默认配置
    WINDOW_PATTERNS = {
    ...
    BORDER_WIDTH = 16
    TITLE_HEIGHT = 48
    MUMU_PROCESS_NAMES = ['MuMuNxDevice.exe', 'MuMuPlayer.exe', 'MuMuVMMHeadless.exe', 'MuMuVMMSVC.exe', 'NemuPlayer.exe']
    MUMU_CLASS_NAMES = ['MuMuPlayer', 'Qt5156QWindowIcon', 'Qt5154QWindowIcon', 'Qt6QWindowIcon', 'Qt5QWindowIcon', 'QWindowIcon']
    EXCLUDE_TITLE_KEYWORDS = ['Qoder', 'PyCharm', 'VSCode', 'Visual Studio', 'IntelliJ', 'Cursor', 'Code', 'Notepad++', 'Sublime']
...
class EmulatorWindowFinder:
    ...
    def _find_by_process(self) -> list:
                    ...
                    for i in range(num_processes):
                        ...
                        try:
                            ...
                            if h_process:
                                ...
                                if ctypes.windll.psapi.EnumProcessModules(h_process, ctypes.byref(h_module), 
                                                                          ctypes.sizeof(h_module), 
                                                                          ctypes.byref(cb_needed_module)):
                                    module_name = ctypes.create_unicode_buffer(260)
                                    if ctypes.windll.psapi.GetModuleBaseNameW(h_process, h_module, module_name, 260):
                                        proc_name = module_name.value
                                        if proc_name in MUMU_PROCESS_NAMES:
                                            mumu_pids.add(pid)
                                            found_processes.append(f"{proc_name}(PID={pid})")
                                kernel32.CloseHandle(h_process)
                        except Exception:
                            continue
            except Exception as e:
        ...
        logger.debug(f"进程名匹配: 找到 {len(mumu_pids)} 个 MuMu 进程: {found_processes}")
        ...
        EXCLUDED_CLASS_NAMES = ['Qt5156QWindowToolSaveBits', 'Qt5154QWindowToolSaveBits', 
                                'Qt6QWindowToolSaveBits', 'Qt5QWindowToolSaveBits',
                                'Qt5156QWindowTool', 'Qt5154QWindowTool']
            ...
            try:
                if win32gui.IsWindowVisible(hwnd):
                    ...
                    if any(excluded in cls_name for excluded in EXCLUDED_CLASS_NAMES):
                        ...
                        title = win32gui.GetWindowText(hwnd)
                        logger.debug(f"类名匹配: 跳过工具窗口 '{title}' (class='{cls_name}')")
                        return
                    ...
                    if any(pattern in cls_name for pattern in MUMU_CLASS_NAMES):
                        ...
                        if HAS_WIN32PROCESS:
                            try:
                                ...
                                if proc.name() not in MUMU_PROCESS_NAMES:
                                    skipped_non_mumu += 1
                                    logger.debug(f"类名匹配: 跳过非 MuMu 进程窗口 '{win32gui.GetWindowText(hwnd)}' (PID={pid}, 进程={proc.name()})")
                        ...
                        if w > 800 and h > 600 and title:
                            results.append((hwnd, title, rect, 'mumu'))
                            logger.debug(f"类名匹配窗口: '{title}' 类名='{cls_name}' ({w}x{h})")
            except Exception as e:
                pass
    ...
    def find_window(self, check_resolution: bool = True, emulator_type: str = None) -> Tuple[int, str, Tuple[int, int, int, int], str]:
        """
    ...
    def _process_candidates(self, candidates: list, check_resolution: bool, match_method: str) -> Optional[Tuple[int, str, Tuple[int, int, int, int], str]]:
        """
        ...
        if not check_resolution:
            ...
            w = rect[2] - rect[0]
            h = rect[3] - rect[1]
            logger.info(f"选择窗口（不检查分辨率）: '{title}' ({w}x{h}) [匹配方式: {match_method}]")
        ...
        if len(candidates) > 1:
            ...
            if valid_windows:
                ...
                raise RuntimeError(
                    ...
                    + self._format_resolution_error(invalid_windows[0][4], invalid_windows[0][5])
                )
    ...
    def _scan_windows_multi(self, windows: list, patterns: list, check_resolution: bool, emu_type: str) -> Optional[Tuple[int, str, Tuple[int, int, int, int]]]:
        """
        扫描多窗口，支持分辨率过滤（兼容旧代码）
        ...
        if len(all_matched) > 1:
            ...
            if valid_windows:
                ...
                raise RuntimeError(
                    ...
                    + self._format_resolution_error(all_matched[0][3], all_matched[0][4])
                )
...
class MuMuConfigManager:
    ...
    def _get_config_file(cls) -> str:
        """获取配置文件路径"""
        PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        return os.path.join(PROJECT_ROOT, 'data', 'mumu_config.json')
                ...
                with open(config_file, 'r', encoding='utf-8') as f:
                    ...
                    if adb_path:
                        ...
                        if os.path.isfile(adb_path):
                            ...
                            logger.warning(f"缓存的 ADB 路径无效（文件不存在）: {adb_path}")
        except (OSError, json.JSONDecodeError) as e:
...
class EmulatorManager:
    ...
    def initialize(self, force_redetect: bool = False, emulator_type: str = None) -> 'MuMuConfig':
        """
        初始化模拟器（查找窗口和端口）
        ...
        port, port_type = self.port_finder.find_port(
            prefer_cached=not force_redetect,
            emulator_type=emulator_type
        )
        if port is None:
        ...
        if detected_type and port_type and detected_type != port_type:
            logger.warning(f"窗口类型 ({detected_type}) 与端口类型 ({port_type}) 不一致")
        ...
        type_display = type_name_map.get(self.emulator_type, self.emulator_type or '未知')
