...
# 本地模块导入
# 导入ADB工具基类
from device.ADBTool import ADBTool
# 从配置导入scrcpy相关参数
from config import SCRCPY_BITRATE, SCRCPY_MAX_FPS, SCRCPY_MAX_SIZE
# 导入日志工具
...
def _patched_stream_loop(self) -> None:
    ...
    while self.alive:
        try:
            ...
            if self.alive:
                self._Client__send_to_listeners(scrcpy.EVENT_DISCONNECT)
                self.stop()
                raise e
...
class DummyStderr:
    ...
    # 要过滤的H.264解码错误关键词列表
    FILTER_KEYWORDS = [
        'sps_id', 'out of range', 'non-existing PPS',
        'no frame', 'decode_slice_header', 'referenced',
        'PPS', 'SPS', 'NAL', 'colocated', 'Frame num change'
    ]
...
class ScrcpyTool(ADBTool):
    ...
    def __init__(self, device_serial: str = None, auto_detect: bool = True):
        """
        初始化 Scrcpy 工具
    ...
    def _check_emulator_window(self):
        ...
        try:
            ...
            # 查找窗口并检查分辨率
            hwnd, title, rect, emu_type = window_finder.find_window(check_resolution=True)
            # 记录检查通过日志
    ...
    def preview(self, fps: int = None):
        ...
        # 设置帧率
        self.client.max_fps = fps if fps is not None else SCRCPY_MAX_FPS
        # 启动客户端，使用独立线程
    ...
    def _on_frame_with_warmup(self, frame):
        ...
        if not self._is_warmed_up:
            ...
            if self._frame_warmup_counter < self.FRAME_WARMUP_COUNT:
                ...
                # 记录预热完成日志
                logger.info(f"[ScrcpyTool] ✓ 帧预热完成，已丢弃{self.FRAME_WARMUP_COUNT}帧，开始正常处理")
