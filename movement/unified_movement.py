...
# 本地模块导入
from config import MINIMAP_SCALE_FACTOR, MOVE_DEADZONE, MOVE_INTERVAL  # 从配置导入移动相关参数
from config import KEY_MOVE_UP, KEY_MOVE_LEFT, KEY_MOVE_DOWN, KEY_MOVE_RIGHT  # 从配置导入方向按键
from config.heroes.mapping import get_hero_chinese  # 导入英雄中文名获取函数
from utils.keyboard_controller import press, release, tap  # 导入键盘按键控制函数
from utils.logging_utils import get_logger  # 导入日志获取函数

import detection.model1_astar_follow as model1_astar_follow  # 导入模态1跟随模块
...
class StuckDetector:
    ...
    def __init__(self):
        ...
        self.time_threshold = 0.5  # 最少需要0.5秒的数据才进行判断（防止刚开始移动就误判）
        ...
        self.avoid_start_time = None   # 绕路开始时间戳
        self.avoid_direction = None    # 当前绕路方向角度（弧度）
        ...
        self.recall_start_time = None        # 回城开始时间
    ...
    def update(self, g_center):
        ...
        self.minimap_history.append((g_center[0], g_center[1], current_time))  # 添加新记录
    ...
    def is_stuck(self, current_time=None):
        ...
        # 两个轴的变化都很小 = 当前帧判定为卡住
        is_currently_stuck = x_range < self.stuck_threshold and y_range < self.stuck_threshold
    ...
    def start_avoidance(self, current_direction):
        ...
        if current_pos:
            ...
            if grid_pos in self.success_exits:
                self.avoid_direction = self.success_exits[grid_pos]
                logger.info(f"检测到卡住 #{self.stuck_count}，使用历史成功方向: {math.degrees(self.avoid_direction):.1f}°")
        ...
        if current_direction is not None:
            for offset_deg in test_offsets_deg:
                ...
                self.avoid_direction = math.atan2(math.sin(test_angle), math.cos(test_angle))
                logger.info(f"检测到卡住 #{self.stuck_count}，尝试偏移{offset_deg}°绕路，角度={math.degrees(self.avoid_direction):.1f}°")
                return self.avoid_direction
            
            # 所有方向都在死路中，只能硬闯原方向
            self.avoid_direction = math.atan2(math.sin(current_direction), math.cos(current_direction))
            logger.warning(f"检测到卡住 #{self.stuck_count}，所有方向都尝试过，沿原方向硬闯")
        else:
            # 没有方向信息时（极端情况），交替尝试上下
            ...
            logger.info(f"检测到卡住 #{self.stuck_count}，无方向信息，向{'上' if self.avoid_direction < 0 else '下'}绕路")
    ...
    def update_avoidance(self):
        ...
        if avoid_duration > 1.5:
            if self.avoid_attempts < self.max_avoid_attempts:
                ...
                # 判断位置是否有明显变化（距离 > 20像素）
                has_moved = False
                if current_pos and hasattr(self, '_avoid_start_pos'):
                ...
                if has_moved:
                    ...
                    logger.debug(f"绕路超时但角色已移动({move_distance:.1f}px)，重置绕路计时器 ({self.avoid_attempts}/{self.max_avoid_attempts})")
                    ...
                    test_offsets_deg = [45, -45, 90, -90, 135, -135, 180]
                    for offset_deg in test_offsets_deg:
                        test_angle = self.avoid_direction + math.radians(offset_deg)
                        # 标准化角度到 [-pi, pi]
                        test_angle = math.atan2(math.sin(test_angle), math.cos(test_angle))
                        # 检查这个方向是否与当前方向明显不同
                        angle_diff = abs(test_angle - self.avoid_direction)
                        if angle_diff > math.radians(30):  # 至少偏离30度
                            self.avoid_direction = test_angle
                            logger.info(f"尝试偏移{offset_deg}°新绕路方向，角度={math.degrees(self.avoid_direction):.1f}°")
                            break
                    return self.avoid_direction
                else:
                    # 角色确实卡住，递增stuck_count并重新开始绕路
                    logger.debug(f"绕路方向无效且角色未移动，尝试新方向 ({self.avoid_attempts}/{self.max_avoid_attempts})")
                    # 获取当前方向作为基准，重新选择绕路方向
                    return self.start_avoidance(self.avoid_direction)
            else:
                # 尝试次数超过上限，放弃绕路
                logger.warning(f"绕路尝试次数超过上限({self.max_avoid_attempts})，放弃绕路")
    ...
    def _end_avoidance_success(self):
        ...
        if self.minimap_history and self.avoid_direction is not None:
            ...
            self.success_exits[grid_pos] = self.avoid_direction  # 记住这个位置的成功方向
    ...
    def get_status(self):
        ...
        """
        return {
            'is_stuck_confirmed': self.stuck_confirm_count >= self.stuck_confirm_required,  # 是否确认卡住
...
class UnifiedMovement:
    ...
    def __init__(self):
        ...
        self.key_status = {KEY_MOVE_UP: False, KEY_MOVE_LEFT: False, KEY_MOVE_DOWN: False, KEY_MOVE_RIGHT: False}  # 当前按键状态
    ...
    def update_from_model2(self, self_pos, team_targets, class_names=None):
        ...
        # 选择最近的队友（无法区分优先英雄）
        closest = min(team_targets, key=lambda t: sqrt((t[0]-self_pos[0])**2 + (t[1]-self_pos[1])**2))
        
        new_pos = (closest[0], closest[1])
        # 检测队友是否在移动（位置变化超过阈值）
        if self._prev_target_pos is not None:
            move_dist = sqrt((new_pos[0] - self._prev_target_pos[0])**2 + (new_pos[1] - self._prev_target_pos[1])**2)
    ...
    def update_from_model1(self, g_center, b_centers, class_names=None):
        ...
        for target in all_targets:
            target_distance = sqrt((target[0]-g_center[0])**2 + (target[1]-g_center[1])**2)
        ...
        if priority_targets:
            # 在优先英雄中选择最近的
            closest = min(priority_targets, key=lambda b: sqrt((b[0]-g_center[0])**2 + (b[1]-g_center[1])**2))
            is_priority = True
        else:
            # 没有优先英雄，选择最近的普通队友
            closest = min(normal_targets, key=lambda b: sqrt((b[0]-g_center[0])**2 + (b[1]-g_center[1])**2))
            is_priority = False
        
        # 检查目标切换保护（防止频繁切换目标）
        current_time = time.time()
        protect_time = self.PRIORITY_TARGET_PROTECT_TIME if self.is_priority_target else self.NORMAL_TARGET_PROTECT_TIME
        in_protect = (current_time - self.target_lock_time) < protect_time
        
        # 判断是否是新目标（位置不同）
        is_new_target = (self.locked_target_pos is None or 
                        abs(self.locked_target_pos[0] - closest[0]) > 10 or 
                        abs(self.locked_target_pos[1] - closest[1]) > 10)
        
        # 在保护期内且是新目标，检查是否值得切换
        if in_protect and is_new_target and self.target_pos is not None:
            # 计算当前目标和新目标的距离
            new_dist = sqrt((closest[0]-g_center[0])**2 + (closest[1]-g_center[1])**2)
            old_dist = sqrt((self.target_pos[0]-g_center[0])**2 + (self.target_pos[1]-g_center[1])**2)
    ...
    def execute_move(self, self_pos, target_pos=None, enemies=None, battle_state=None):
        ...
        if self.stuck_detector.is_recalling:
            elapsed = current_time - self.stuck_detector.recall_start_time
        ...
        if (self.stuck_detector.stuck_count >= self.stuck_detector.STUCK_RECALL_THRESHOLD 
        ...
        if battle_state is not None and battle_state == 'retreat' and enemies and len(enemies) > 0:
            # 计算所有敌人的平均位置
            enemy_positions = [(e[0], e[1]) for e in enemies]  # enemies格式: [(x, y, health%), ...]
            ...
            if retreat_dist > 1:
                ...
                if hasattr(self, 'logger') and self.logger:
                    self.logger.info(f"[RETREAT] 远离敌人移动, 敌人中心=({avg_enemy_x:.0f},{avg_enemy_y:.0f}), 方向=({dx:.0f},{dy:.0f})")
        ...
        if original_distance < MIN_FOLLOW_DISTANCE and not self._target_moving:
            ...
            if (current_time - self._last_log_time) > 1.0:  # 每秒输出一次
                logger.debug(f"距离队友过近({int(original_distance)}px)，队友静止，停止移动避免遮挡血条")
        ...
        if current_time - self._last_stuck_check_log > 3.0:
            ...
            logger.debug(f"[卡地形检查] is_stuck={is_stuck_result}, 历史={stuck_status['history_size']}, 确认={stuck_status['stuck_confirm_count']}, stuck_count={stuck_status['stuck_count']}, 阈值={self.stuck_detector.stuck_threshold}")
        ...
        # 计算移动方向（8方向）
        new_keys = {KEY_MOVE_UP: False, KEY_MOVE_LEFT: False, KEY_MOVE_DOWN: False, KEY_MOVE_RIGHT: False}
        ...
        if active_keys and (current_time - self._last_log_time) > 1.0:
            priority_mark = "⭐优先" if getattr(self, 'is_priority_target', False) else ""  # 优先目标标记
            avoid_mark = "🔄绕路" if is_avoiding else ""  # 绕路标记
            pos_str = f"({int(self.target_pos[0])},{int(self.target_pos[1])})" if self.target_pos is not None else "(None)"
            logger.debug(f"[移动指令] {'+'.join(active_keys)} -> {self.target_name}{priority_mark}{avoid_mark}{pos_str} [{self.source}]")
            self._last_log_time = current_time
        ...
        if current_time - self._last_stuck_log_time > 5.0:
            ...
            if status['stuck_count'] > 0 or status['is_avoiding']:
                logger.debug(f"[卡地形状态] 历史次数:{status['stuck_count']}, 成功脱困:{status['avoid_success']}, 正在绕路:{status['is_avoiding']}")
