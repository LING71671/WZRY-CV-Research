...
from utils.keyboard_controller import press, release  # 导入键盘按键控制函数
from utils.logging_utils import setup_colored_logger  # 导入彩色日志设置函数
from utils.utils import find_closest_target, safe_queue_put  # 导入工具函数
...
class BaseSupportHero:
    ...
    def __init__(self, skill_queue: Queue = None, pause_event: threading.Event = None):
        """
        初始化跟随型辅助
        ...
        """
        self.logger = setup_colored_logger(f"{self.HERO_NAME}_follow")  # 创建带颜色的日志记录器
        self.skill_queue = skill_queue or Queue(maxsize=1)  # 技能队列，用于与技能逻辑通信
    ...
    def find_closest_target(self, self_pos: Tuple[float, float],
                            targets: List[Tuple]) -> Optional[Tuple]:
        """
        找到最近的队友目标
    ...
    def calculate_move_direction(self, self_pos: Tuple[float, float],
                                  target_pos: Tuple[float, float],
                                  enemies: List[Tuple] = None) -> Tuple[float, float]:
        """
        计算移动方向，考虑避敌
        ...
        if enemies and self_pos:
            ...
            if dist_enemy < self.SAFE_DISTANCE:
                ...
                self.logger.debug(f"[{self.HERO_NAME}] 避敌调整: 敌人距离={dist_enemy:.0f}px")
    ...
    def process_mode2_movement(self, detection: Dict) -> Dict:
        ...
        if self_pos and team_targets:
            ...
            if closest:
                dx, dy = self.calculate_move_direction(self_pos, closest[:2], enemies)  # 计算移动方向
                ...
                if distance > self.FOLLOW_DISTANCE:
                    ...
                    self.logger.debug(f"[{self.HERO_NAME}] 跟随移动: 距离={distance:.0f}px")
    ...
    def process_mode1_movement(self, detection: Dict) -> Dict:
        ...
        if g_center and b_centers:
            ...
            health_info['is_moving'] = movement.get('is_moving', False)  # 记录是否正在移动
        else:
            self._release_all_keys()  # 没有目标时释放按键
            
        return health_info
    
    def check_mode_switch(self, model1_result: Optional[Tuple], 
                          model2_result: Optional[Tuple]) -> int:
        """
        检查是否需要切换模态（模态1/模态2）
        根据检测结果决定使用哪种模态进行跟随
        ...
        if current_time - self.last_check_time >= 10:
            ...
            if model2_result:
                detection = model2_result[0] if isinstance(model2_result, tuple) else model2_result
                enemies = detection.get('enemies', [])
                
                # 如果没有敌人且模态1有结果，检查是否有优先英雄
                if not enemies and model1_result:
                    m1_det = model1_result[0] if isinstance(model1_result, tuple) else model1_result
                    ...
                    for b in b_centers:
                        if len(b) > 2:
                            ...
                            if hero_name in model1_astar_follow.priority_heroes:
                                self.force_mode1_until = current_time + 10  # 强制使用模态1 10秒
                                self.logger.info(f"[{self.HERO_NAME}] 检测到优先英雄，强制模态1")
                                return 1
    ...
    def update_game_state(self, model1_result: Optional[Tuple], 
                          model2_result: Optional[Tuple]) -> bool:
        """
        更新游戏状态（检测游戏是否进行中）
        ...
        if model2_result:
            m2_det = model2_result[0] if isinstance(model2_result, tuple) else model2_result
    ...
    def send_health_info(self, health_info: Dict, status_queue: Queue = None):
        """
        发送健康信息到技能队列和状态队列
        只在信息变化时发送，避免重复
    ...
    @abstractmethod
    def get_skill_logic_function(self):
        """
        获取英雄特定的技能逻辑函数
        子类必须实现此方法
    ...
    def run(self, status_queue: Queue = None):
        """
        运行跟随逻辑主循环
        子类可以覆盖此方法实现自定义逻辑
...
class YaoFollowLogic(BaseSupportHero):
    ...
    def get_skill_logic_function(self):
        from skills.yao_skill_logic_v2 import YaoSkillLogic
...
class CaiwenjiFollowLogic(BaseSupportHero):
    ...
    def get_skill_logic_function(self):
        from skills.caiwenji_skill_logic_v2 import CaiwenjiSkillLogic
        return lambda q: None  # V2版本使用类方式，此处返回空函数兼容处理
...
class MingshiyinFollowLogic(BaseSupportHero):
    ...
    def get_skill_logic_function(self):
        from skills.mingshiyin_skill_logic_v2 import MingshiyinSkillLogic
        return lambda q: None  # V2版本使用类方式，此处返回空函数兼容处理


# 英雄工厂函数：根据英雄名称创建对应的跟随逻辑实例
def create_support_hero(hero_name: str, skill_queue: Queue = None, 
                        pause_event: threading.Event = None) -> BaseSupportHero:
    """
    根据英雄名称创建对应的跟随逻辑实例
