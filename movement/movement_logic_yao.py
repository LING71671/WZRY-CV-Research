...
from config.heroes.mapping import convert_priority_heroes, get_hero_chinese  # 导入英雄名称转换函数
from utils.utils import safe_queue_put, find_closest_target  # 导入工具函数

# 战斗AI四层架构
from battle.world_state import WorldStateBuilder
from battle.threat_analyzer import ThreatAnalyzer
...
# 导入检测模块
import detection.model1_detector as model1_detector  # 模态1检测器
import detection.model2_detector as model2_detector  # 模态2检测器
import detection.model1_astar_follow as model1_astar_follow  # 模态1跟随逻辑
from utils.keyboard_controller import press, release  # 键盘控制函数

# 从统一按键配置导入移动方向按键
from config.keys import KEY_MOVE_UP, KEY_MOVE_DOWN, KEY_MOVE_LEFT, KEY_MOVE_RIGHT

# 导入统一移动控制器
from movement.unified_movement import get_movement_controller
...
def run_fusion_logic_v2(skill_queue, pause_event, status_queue=None, 
                        model1_data_queue=None, model2_data_queue=None,
                        decision_maker=None):
    """
    ...
    while True:
        try:
                ...
                if g_center:
                    with _model1_data_lock:
                        ...
                        latest_model1_data.update({
                            'g_center': g_center,
                            'b_centers': b_centers,
                            'r_centers': model1_result.get('r_centers', [])  # 敌方英雄列表
                        })
                    # 更新统一移动控制器（模态1优先级低）
                    # 注意：卡地形检测器在 update_from_model1 内部更新
                    if not is_paused:
                        movement.update_from_model1(g_center, b_centers, model1_astar_follow.class_names)
                        # 调试日志：确认卡地形检测器已更新
                        if not hasattr(run_fusion_logic_v2, '_last_stuck_update_log'):
                            run_fusion_logic_v2._last_stuck_update_log = 0
                        if current_time - run_fusion_logic_v2._last_stuck_update_log > 3.0:
                            stuck_status = movement.stuck_detector.get_status()
                            logger.debug(f"[卡地形调试] 历史大小:{stuck_status['history_size']}, 确认计数:{stuck_status['stuck_confirm_count']}, 绕路中:{stuck_status['is_avoiding']}")
                            run_fusion_logic_v2._last_stuck_update_log = current_time
            ...
            if model2_result:
                ...
                is_attached = (self_health is None and self_pos is not None)  # 判断是否附身
                
                # 更新统一移动控制器（模态2优先级高）
                if not is_paused and self_pos:
                    if team_targets:
                        movement.update_from_model2(self_pos, team_targets, model1_astar_follow.class_names)
                    else:
                ...
                health_info = {
                    ...
                    'team_health': [{'pos': t[:2], 'health': t[2]} for t in team_targets],  # 队友血量列表
                    'enemy_health': [{'pos': e[:2], 'health': e[2]} for e in enemies],  # 敌人血量列表
                ...
                world_state = world_builder.build(model1_result, model2_result, current_time)
                ...
                if current_time - last_cognition_time >= COGNITION_INTERVAL:
                    ...
                    current_focus_target = threat_analyzer.detect_focus_fire(world_state)
                ...
                health_info['battle_state'] = current_battle_state.value if current_battle_state else 'follow'
                health_info['threat_level'] = current_threat_level.value if current_threat_level else 'safe'
                health_info['skill_policy'] = battle_fsm.get_skill_policy() if current_battle_state else 'aggressive'
                ...
                if skill_queue and not movement.stuck_detector.is_recalling:
                    try:
                        ...
                        # 调试日志：每2秒输出一次发送状态
                        if not hasattr(run_fusion_logic_v2, '_last_skill_queue_log'):
                            run_fusion_logic_v2._last_skill_queue_log = 0
                        if current_time - run_fusion_logic_v2._last_skill_queue_log > 2:
                            logger.debug(f"[Fusion V2] 发送技能队列: 自身血量={self_health}, 队友数={len(team_targets)}, 敌人数={len(enemies)}")
                            run_fusion_logic_v2._last_skill_queue_log = current_time
                    except Empty:
                        pass
                ...
                if model2_result and model2_result.get('self_health') is None and model2_result.get('self_pos'):
                    is_currently_attached = True
                
                if is_currently_attached:
                    last_attached_time = current_time  # 更新附身时间
                
                in_attach_protection = (current_time - last_attached_time < ATTACHED_PROTECT_DURATION)
                ...
                if has_team_targets and current_mode != 2:
                    ...
                    current_mode = 2
                elif not has_team_targets and current_mode == 2 and not in_attach_protection:
                ...
                if not is_paused:
                    ...
                    if g_center_for_stuck and not has_team_targets and not is_currently_attached:
                    ...
                    if current_mode == 2 and self_pos and team_targets:
                        # Mode2：有队友时才执行移动
                        movement.execute_move(self_pos, enemies=enemies, battle_state=current_battle_state.value if current_battle_state else None)
                        ...
                        if g_center and b_centers:
                            movement.update_from_model1(g_center, b_centers, model1_astar_follow.class_names)
                            movement.execute_move(g_center, enemies=enemies, battle_state=current_battle_state.value if current_battle_state else None)
                        elif g_center:
                            # 有自身位置但没有队友，朝右下方向移动（用于卡地形检测）
                            # 目标点 (330, 210) 在小地图右下方，产生向右下的移动向量
                            target_point = (330, 210)
                            movement.execute_move(g_center, target_pos=target_point, enemies=enemies, battle_state=current_battle_state.value if current_battle_state else None)
                        else:
                        ...
                        if g_center_for_stuck:
                            # 朝右下方向移动
                            target_point = (330, 210)
                            movement.execute_move(g_center_for_stuck, target_pos=target_point, enemies=enemies, battle_state=current_battle_state.value if current_battle_state else None)
                        else:
                            movement.clear()  # 停止移动
                else:
                    # 暂停时释放按键
