...
class EntityState:
    ...
    pos: tuple                  # 当前坐标 (x, y)，使用屏幕像素坐标
    health: int                 # 血量百分比 0-100
    health_delta: float = 0.0   # 血量变化率（%/秒），负值=掉血
    velocity: tuple = (0.0, 0.0)  # 速度向量 (vx, vy) 像素/秒
...
class WorldState:
    ...
    self_pos: tuple = None       # 自身位置 (x, y)
    self_health: int = None      # 自身血量百分比
    self_health_delta: float = 0.0  # 自身血量变化率
    is_attached: bool = False    # 是否附身
    teammates: list = field(default_factory=list)   # List[EntityState]
    enemies: list = field(default_factory=list)      # List[EntityState]
...
class WorldStateBuilder:
    ...
    def __init__(self):
        ...
        初始化缓存结构，用于存储上一帧的数据。
        """
        self._prev_self = None  # (pos, health, timestamp)
        self._prev_teammates = []  # [(pos, health, timestamp), ...]
        self._prev_enemies = []    # [(pos, health, timestamp), ...]
        logger.debug("WorldStateBuilder 初始化完成")
    
    def build(self, model1_result: Optional[Dict], model2_result: Optional[Dict], 
              current_time: float) -> WorldState:
        """
        构建世界状态
        ...
        self_health = model2_result.get('self_health')
        ...
        if is_attached:
            self_health = 100  # 附身时无法获取自身血量，设为默认值
        
        # 计算自身速度和血量变化率
        self_velocity = (0.0, 0.0)
        self_health_delta = 0.0
        
        if self_pos is not None and self._prev_self is not None:
            ...
            if dt >= 0.001:  # 防止除零
                ...
                # 计算血量变化率
                self_health_delta = (self_health - prev_health) / dt
        ...
        if self_pos is not None:
            self._prev_self = (self_pos, self_health, current_time)
    ...
    def _build_entity_list(self, m2_entities: List, m1_entities: List,
                           class_names: Dict, self_pos: Optional[Tuple],
                           current_time: float, prev_entities: List,
                           is_teammate: bool) -> List[EntityState]:
        """
        构建实体列表
        ...
        for idx, entity in enumerate(m2_entities):
            ...
            if matched_prev is not None:
                ...
                if dt >= 0.001:  # 防止除零
                    ...
                    ) if self_pos is not None else 0.0
    ...
    def _match_previous_entity(self, pos: Tuple, prev_entities: List, 
                               used_indices: set) -> Optional[Tuple]:
        """
        匹配上一帧的实体
    ...
    def _get_entity_id(self, idx: int, pos: Tuple, m1_entities: List, 
    ...
    def _infer_lane_role(self, entity_id: str) -> str:
        ...
        try:
            # 导入必要的模块
            from config.heroes.mapping import get_hero_chinese, LANE_HEROES
