...
from utils.frame_manager import get_full_frame
from utils.logging_utils import get_logger
from utils.utils import get_cuda_device
...
def clear_entity_cache():
    ...
    返回值说明：
        无返回值
    """
    if hasattr(HealthBar, '_team_health_history'):
        HealthBar._team_health_history.clear()
...
class HealthBar:
    ...
    def calculate_health_percentage(self, roi_hsv, detected_width, debug=False):
        ...
        """
        # 检查 ROI 是否有效（宽高必须大于0）
        if roi_hsv is None or roi_hsv.size == 0 or roi_hsv.shape[0] < 1 or roi_hsv.shape[1] < 1:
            if debug:
        ...
        if best_health is not None:
            if debug:
                logger.debug(f"[血量计算] 最大匹配轮廓宽度:{best_w}px, 总宽度:{detected_width}px, 血量:{best_health}%")
            # 95%~100% 视为满血，消除微小误差
...
green_health_bar = HealthBar(
    ...
    target_height=10, width_tolerance=5, height_tolerance=5,
    color=(0, 255, 0), label='Self'
)
...
blue_health_bar = HealthBar(
    ...
    target_height=13, width_tolerance=15, height_tolerance=8,
    color=(255, 0, 0), label='Team'
)
...
red_health_bar = HealthBar(
    name="Red Health Bar",
    lower_hsv=[np.array([0, 40, 120]), np.array([170, 40, 120])],    # 两段：0~8 和 170~180
    upper_hsv=[np.array([8, 255, 255]), np.array([180, 255, 255])],   # 覆盖红色完整 hue 范围
    target_height=13, width_tolerance=5, height_tolerance=4,
...
def detect(sct=None, frame=None):
    ...
    results = model.track(img_for_yolo, conf=YOLO_CONF, iou=YOLO_IOU, persist=True, verbose=False)
    ...
    # 血量历史记录清理时间戳
    if not hasattr(detect, '_last_health_cleanup_time'):
        detect._last_health_cleanup_time = 0
    ...
    for result in results:
        for box in result.boxes:
            ...
            raw_health_percentage = health_bar.calculate_health_percentage(roi_hsv, detected_width, debug=debug_mode)
            ...
            if track_id >= 0:
                # 初始化全局血量历史字典（所有实体共用）
                if not hasattr(HealthBar, '_team_health_history'):
                    HealthBar._team_health_history = {}
                
                # 使用 track_id 作为键（比坐标更稳定）
                pos_key = f"track_{track_id}"
                last_health = HealthBar._team_health_history.get(pos_key, raw_health_percentage)
                ...
                if delta < -40:
                    ...
                    health_percentage = int(last_health * 0.3 + raw_health_percentage * 0.7)
                elif delta > 30:
                    # 血量突然大幅上升（可能是误检/切换目标）：保守更新
                    health_percentage = int(last_health * 0.8 + raw_health_percentage * 0.2)
                else:
                    # 正常波动范围：标准 EMA 平滑
                    health_percentage = int(last_health * 0.6 + raw_health_percentage * 0.4)
                
                # 始终更新历史值（旧版 bug：大幅变化时不更新导致血量卡住）
                HealthBar._team_health_history[pos_key] = health_percentage
            
            # 添加到当前帧检测列表（包含 track_id）
            current_detections.append((center_x, center_y, health_percentage, entity_type, track_id))
    ...
    # 定期清理血量历史记录（每10秒）
    if current_time - detect._last_health_cleanup_time >= 10.0:
        detect._last_health_cleanup_time = current_time
        if hasattr(HealthBar, '_team_health_history'):
            # 只保留当前活跃的 track_id 对应的历史记录
            keys_to_remove = []
            for key in HealthBar._team_health_history:
            ...
            for key in keys_to_remove:
                del HealthBar._team_health_history[key]
    ...
    if len(self_candidates) == 1:
        ...
        for candidate in self_candidates:
            if candidate != best_candidate:
                ...
                log_details['team'].append((x, y, health, is_estimated))
                team_idx += 1
        logger.debug(f"多 self 筛选: 从 {len(self_candidates)} 个候选中选择了最近的作为自身，其余 {len(self_candidates)-1} 个降级为队友")

    # 缩放为50%以提高显示性能
    img_resized = cv2.resize(img, (img.shape[1] // 2, img.shape[0] // 2))
    
    # 控制日志输出频率（每1秒一次）
    if not hasattr(detect, '_last_log_time'):
        detect._last_log_time = 0
    should_log = current_time - detect._last_log_time >= 1.0
    
    if should_log:
        detect._last_log_time = current_time
        # 输出检测汇总日志
        total = detection_count['self'] + detection_count['team'] + detection_count['enemy']
        if total > 0:
            estimated_count = sum(1 for e in tracked_entities if e[4])
            logger.info(f"检测汇总: 总计{total}个血条 (自身:{detection_count['self']}, 队友:{detection_count['team']}, 敌人:{detection_count['enemy']}), 估计位置:{estimated_count}")
        else:
    ...
    if self_pos is None and team_targets:
        ...
        if closest_to_center:
            ...
            self_health = None
            if should_log:
                logger.debug(f"瑶附身推断: 使用队友位置作为自身位置 ({self_pos[0]:.1f}, {self_pos[1]:.1f})")
