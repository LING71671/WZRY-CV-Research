...
from config.base import MODEL3_WEIGHTS, MODEL3_CONFIDENCE_THRESHOLD, YOLO_IOU
...
def detect(frame=None):
    ...
    try:
        ...
        results = model.predict(img_for_yolo, conf=MODEL3_CONFIDENCE_THRESHOLD, iou=YOLO_IOU, verbose=False)
        ...
        for r in results:
            ...
            for box in r.boxes:
                ...
                if cls_id in MODEL3_CLASSES:
                    ...
                    cv2.putText(img, label, (x1, label_y), cv2.FONT_HERSHEY_SIMPLEX, 
                               0.6, (255, 255, 255), 2)
        ...
        if result['events']:
            ...
            cv2.putText(img, "No Events", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 
                       1.0, (128, 128, 128), 2)
