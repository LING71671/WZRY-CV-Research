...
from utils.frame_manager import get_minimap_frame
from config.heroes.mapping import get_hero_chinese
from utils.logging_utils import get_logger
from utils.utils import get_cuda_device, cv2_add_chinese_text
...
class MinimapTracker:
    ...
    def update(self, detections, current_time):
        ...
        for track_id, track in self.tracks.items():
            ...
            if smoothed_pos is not None:
                result.append((int(smoothed_pos[0]), int(smoothed_pos[1]), track['class_id']))
    ...
    def _smooth_position(self, positions):
        ...
        weighted_x = sum(pos[0] * w for pos, w in zip(positions, weights)) / total_weight
        weighted_y = sum(pos[1] * w for pos, w in zip(positions, weights)) / total_weight
...
def detect(sct=None, frame=None, visualize=True):
    ...
    """
    global _last_yao_position, _yao_lost_frames, _team_tracker, _enemy_tracker, _self_position_buffer
    ...
    if _cached_detect_result is not None and _skip_frame_counter % _skip_frame_interval != 0:
        # 非推理帧：复用缓存结果，但更新可视化帧
    ...
    if results[0].boxes is not None and len(results[0].boxes) > 0:
        ...
        for i, box in enumerate(boxes):
            ...
            if confidence > confidence_threshold:
                ...
                detections.append({
                    'x': x, 'y': y, 'class_id': class_id, 'class_name': class_name,
                    'is_yao': is_yao, 'is_green': is_green
                })
        ...
        if g_center is None and _last_yao_position is not None and _yao_lost_frames < MAX_YAO_LOST_FRAMES:
        ...
        if should_log and detected_info:
            ...
            for info in detected_info:
                ...
                log_msg += f" [{display_name}]({info['x']},{info['y']})置信{info['conf']:.2f}"
            logger.info(log_msg)
    ...
    if g_center is not None:
        ...
        if len(_self_position_buffer) > 0:
            ...
            total_weight = sum(weights)
            smoothed_x = sum(pos[0] * w for pos, w in zip(_self_position_buffer, weights)) / total_weight
            smoothed_y = sum(pos[1] * w for pos, w in zip(_self_position_buffer, weights)) / total_weight
    ...
    if should_log and (len(b_centers_smoothed) != len(b_centers) or len(r_centers_smoothed) != len(r_centers)):
        logger.debug(f"多帧平滑: 队友 {len(b_centers)}->{len(b_centers_smoothed)}, 敌人 {len(r_centers)}->{len(r_centers_smoothed)}")
    ...
    if g_center is None and _last_yao_position is not None:
        ...
        if _attached_proxy_track_id is None:
            ...
            for track_id, track in _team_tracker.tracks.items():
                ...
                last_pos = track['positions'][-1]
                tx, ty = last_pos[0], last_pos[1]
                dist = np.sqrt((tx - _last_yao_position[0])**2 + (ty - _last_yao_position[1])**2)
            ...
            if best_track_id is not None:
                ...
                logger.info(f"[附身代理] 锁定队友轨迹 {best_track_id}，"
        ...
        if _attached_proxy_track_id is not None:
            if _attached_proxy_track_id in _team_tracker.tracks:
                ...
                if smoothed is not None:
                    ...
                    if should_log:
                        logger.debug(f"[附身代理] 使用队友位置 ({g_center[0]},{g_center[1]})")
            else:
                # 队友轨迹已超时消失，释放代理
                logger.info(f"[附身代理] 队友轨迹 {_attached_proxy_track_id} 已消失，释放代理")
                _attached_proxy_track_id = None
                _proxy_lock_time = None
            
            # 超时保护：代理持续时间过长则释放（防止跟错人）
            if _proxy_lock_time is not None and current_time - _proxy_lock_time > PROXY_MAX_DURATION:
    ...
    try:
        while True:
            if latest_frame[0] is not None:
                ...
                base_w, base_h = DEFAULT_REGIONS['full']['width'], DEFAULT_REGIONS['full']['height']
