...
def _get_template_confidence(template_name: str) -> float:
    ...
    return TEMPLATE_CONFIDENCE_THRESHOLDS.get(template_name, DEFAULT_TEMPLATE_CONFIDENCE)
...
class DeepDetectionStrategy(DetectionStrategy):
    ...
    def detect(self, img_gray, template_matcher, context: dict) -> Optional[tuple]:
        """
        扫描所有状态签名中的模板
        ...
        for template_name in state_templates:
            ...
            result = template_matcher.detect(template_name, img_gray, min_confidence=0.3)
            # 如果找到匹配，添加到候选列表
            if result.found:
        ...
        for template_name, confidence in candidates:
            ...
            elif not isinstance(required_confidence, (int, float)) or required_confidence is None:
            ...
            if exclude_templates:
                ...
                for excl_tmpl in exclude_templates:
                    excl_result = template_matcher.detect(excl_tmpl, img_gray, min_confidence=0.5)
                    if excl_result.found:
                        logger.debug(f"深度检测: 状态 {candidate_state.value} 被排除模板 {excl_tmpl} 排除 (conf={excl_result.confidence:.3f})")
                        excluded = True
                        break
    ...
    def detect(self, img_gray, template_matcher, context: dict) -> Optional[str]:
        ...
        for parent_mode, config in TEMPLATE_CONFIDENCE_THRESHOLDS.items():
            ...
            for template in templates:
                # 执行模板匹配
                result = template_matcher.detect(template, img_gray, min_confidence=threshold)
                # 对关键模板输出调试日志
                if template in ['wzry_icon', 'game_lobby', 'start_game', 'battle_mode', 'wangzhe_canyon']:
                    throttled_logger.debug(f"[{group_name}] 检测 {template}: found={result.found}, conf={result.confidence:.3f}, threshold={threshold}")
                # 收集检测日志（延迟输出）
                if group_idx > 1:
                    _all_debug_logs.append(f"[{group_name}] 检测 {template}: found={result.found}, conf={result.confidence:.3f}, threshold={threshold}")
                # 如果模板匹配成功
                if result.found and result.confidence > threshold:
                    ...
                    if exclude_templates:
                        ...
                        # 逐一检测排除模板
                        for excl_tmpl in exclude_templates:
                            excl_result = template_matcher.detect(excl_tmpl, img_gray, min_confidence=0.70)
                            if excl_result.found:
                                logger.info(f"  ⚠ 排除模板 {excl_tmpl} 匹配 (conf={excl_result.confidence:.3f})，跳过 {parent_mode}")
                                excluded = True
                                break
                        # 如果存在排除模板匹配，跳过此母模式
                        if excluded:
        ...
        if sub_modes:
            ...
            for sub_mode, sub_threshold in sub_modes.items():
                # 执行子模式模板检测
                result = template_matcher.detect(sub_mode, img_gray, min_confidence=sub_threshold)
                # 判断子模式是否处于选中状态（高亮状态）
            ...
            if detected_sub_modes:
                # 子模式名称映射表，用于日志显示
                mode_names = {
                    'ai_standard': '标准模式', 'ai_quick': '快速模式',
                    'ai_recommend': '推荐', 'ai_bronze': '青铜', 'ai_gold': '黄金',
                    'ai_diamond': '钻石', 'ai_star': '星耀', 'ai_master': '王者',
                    'start_practice': '开始练习'
                }
                # 将检测到的子模式转换为中文名称
                selected_modes = [mode_names.get(k, k) for k in detected_sub_modes.keys()]
        ...
        if best_parent in HERO_AVATAR_PARENT_MODES:
            ...
            for hero_key in SKILL_VERIFICATION_HEROES:
                ...
                # 检测技能图标
                result = template_matcher.detect(skill_template, img_gray, min_confidence=SKILL_VERIFICATION_THRESHOLD)
                if result.found:
                    # 技能验证通过，更新最佳验证结果
            ...
            if verified_hero:
                ...
                logger.info(f"技能验证: {verified_hero} (置信度: {verified_confidence:.3f})")
        else:
            # 不在选英雄界面，禁用英雄头像检测
...
class DetectionStrategyFactory:
    ...
    @staticmethod
    def create_strategy(level: DetectionLevel, current_state: str = None) -> DetectionStrategy:
        """
        创建指定级别的检测策略
