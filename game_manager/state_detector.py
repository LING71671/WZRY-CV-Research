...
# 从日志工具模块导入日志记录器
from utils.logging_utils import get_logger, ThrottledLogger
...
# 从本地模块导入状态定义相关
from .state_definitions import GameState, STATE_SIGNATURES, get_state_description, get_state_flow
# 从本地模块导入状态转换相关
...
class GameStateDetector:
    ...
    def __init__(self, template_matcher, click_executor,
                 history_size: int = 5, 
    ...
    def _click_match_confirm_with_verification(self, click_x: int, click_y: int, img_gray) -> bool:
        """
        点击匹配确认按钮并验证点击是否生效
        ...
        for attempt in range(max_retries):
            # 执行点击操作
            logger.info(f"匹配成功，点击确认按钮: ({click_x}, {click_y}) [尝试 {attempt + 1}/{max_retries}]")
            ...
            if hasattr(self.template_matcher, '_last_frame') and self.template_matcher._last_frame is not None:
            ...
            if confirmed_result.found and confirmed_result.confidence > verification_threshold:
                logger.info(f"✓ 匹配确认点击验证成功（按钮已变灰）置信度: {confirmed_result.confidence:.2f}")
                return True
            ...
            if match_confirm_result.found:
                if attempt < max_retries - 1:
                    # 还有重试次数，等待后重试
                    logger.warning(f"⚠ 匹配确认点击未生效，重试点击... [尝试 {attempt + 1}/{max_retries}]")
                    time.sleep(0.3)
                else:
                    # 已用完所有重试次数
    ...
    def _check_state_stuck(self, current_state: str) -> bool:
        ...
        # 验证：检测是否有任何母模板匹配
        from config import TEMPLATE_CONFIDENCE_THRESHOLDS
        any_detected = False
        ...
        for parent_mode, config in TEMPLATE_CONFIDENCE_THRESHOLDS.items():
            ...
            # 处理单个模板或模板列表
            templates = [parent_template] if isinstance(parent_template, str) else parent_template
            threshold = config.get('threshold', 0.8)
            
            # 检测该母模式的所有模板
            for template in templates:
                result = self.template_matcher.detect(template, self._last_frame, min_confidence=threshold)
        ...
        if any_detected:
            ...
            if self.stuck_verify_counter >= STATE_STUCK_VERIFY_COUNT:
                # 连续多次未检测到界面，确认停滞
                logger.warning(f"⚠️ 状态停滞验证完成，连续{STATE_STUCK_VERIFY_COUNT}次未检测到界面")
                self.stuck_verify_counter = 0  # 重置计数器
                return True
            return False
    
    def _scan_for_popups(self, img_gray, current_state: str = None) -> Tuple[bool, str]:
        """
        扫描弹窗元素
        ...
        popup_templates = [
            'close1', 'close2', 'close3', 'close4',  # 各种关闭按钮
        ...
        for template in popup_templates:
            result = self.template_matcher.detect(
                template, 
                img_gray,
                min_confidence=POPUP_DETECTION_THRESHOLD
            )
            if result.found:
                # 发现弹窗，输出日志并返回
    ...
    def _handle_stuck_state(self, img_gray, current_state: str = None) -> bool:
        """
        处理状态停滞 - 扫描并点击弹窗
    ...
    def detect(self, img_gray, img_bgr=None, allow_popup_handle: bool = True) -> DetectionResult:
        """
        主检测入口
        ...
        # ========== 2. 状态确认（防抖） ==========
        confirmed_state, is_confirmed = self._confirm_state(detected_state, confidence, strategy)
        ...
        if (confirmed_state != GameState.UNKNOWN.value and 
            confirmed_state not in TRANSITION_STATES and 
            allow_popup_handle):
        ...
        if confirmed_state == GameState.BATTLE_AI_MODE.value:
            ...
            for tmpl_name, mode_name in mode_templates.items():
                result = self.template_matcher.detect(tmpl_name, img_gray, min_confidence=0.50)
                logger.info(f"模式检测: {tmpl_name} found={result.found}, conf={result.confidence:.3f}")
            ...
            difficulty_templates = {
                'ai_recommend': '推荐', 'ai_bronze': '青铜', 'ai_gold': '黄金',
                'ai_diamond': '钻石', 'ai_star': '星耀', 'ai_master': '王者'
            }
            best_diff_name = None
            best_diff_conf = 0.0
            ...
            for tmpl_name, diff_name in difficulty_templates.items():
                result = self.template_matcher.detect(tmpl_name, img_gray, min_confidence=0.30)
                logger.info(f"难度检测: {tmpl_name} found={result.found}, conf={result.confidence:.3f}")
            ...
            if selected_diff and selected_conf >= 0.40:
                ...
                logger.info(f"选中难度: {selected_diff} (conf={selected_conf:.3f}, found_match={'是' if best_found_name else '否'})")
            ...
            logger.info(f"当前选中: 模式={current_mode}, 难度={current_difficulty}, 所有检测到的难度={all_difficulties}")
            # 调用AI模式处理器检查配置并自动调整
            ...
            if can_proceed and AUTO_LAUNCH:
                from config import TEMPLATE_ROI
                ...
                if roi:
                    ...
                    try:
                        logger.info(f"配置正确，直接点击开始练习: ({click_x}, {click_y})")
                        success = self.click_executor.click(click_x, click_y)
                    except (AttributeError, OSError, RuntimeError) as e:
        ...
        if confirmed_state == GameState.LAUNCHER.value and AUTO_LAUNCH:
            # 检测游戏图标位置并点击
            result = self.template_matcher.detect('wzry_icon', img_gray, min_confidence=0.70)
            if result.found:
                # 计算图标中心点坐标
                x, y = result.location
                w, h = result.size
        ...
        if confirmed_state == GameState.HERO_SELECT.value:
            ...
            select_result = self.hero_selector.select_hero_by_priority(img_gray, img_bgr)
        ...
        if confirmed_state == GameState.HALL.value and self._try_action('hall'):
            from config import TEMPLATE_ROI, GAME_MODE
            ...
            if roi:
                ...
                click_y = roi['y'] + roi['h'] // 2
                try:
                    logger.info(f"游戏大厅，直接点击{action_desc}: ({click_x}, {click_y})")
                    success = self.click_executor.click(click_x, click_y)
                    if success:
        ...
        if confirmed_state == GameState.BATTLE_MODE_SELECT.value and self._try_action('battle_mode_select'):
            from config import TEMPLATE_ROI
        ...
        if confirmed_state == GameState.RANKING_HERO_SELECT.value:
            ...
            if select_result.success:
                ...
                details['action'] = f'ranking_hero_selected_{select_result.selected_hero}'
            else:
                details['hero_select_result'] = {
                    'success': False,
                    'message': select_result.message
        ...
        if confirmed_state == GameState.BATTLE_5V5_SUB.value and self._try_action('battle_5v5_sub'):
            from config import V5_MODE, TEMPLATE_ROI
            ...
            if roi:
                ...
                try:
                    logger.info(f"王者峡谷界面，直接点击{action_desc}: ({click_x}, {click_y})")
                    success = self.click_executor.click(click_x, click_y)
                    if success:
        ...
        if confirmed_state == GameState.BATTLE_5V5_TYPE.value and self._try_action('battle_5v5_type'):
            from config import TEMPLATE_ROI
            ...
            if roi:
                ...
                try:
                    logger.info(f"对战类型选择，直接点击人机模式: ({click_x}, {click_y})")
                    success = self.click_executor.click(click_x, click_y)
                    if success:
        ...
        if confirmed_state in (GameState.BATTLE_AI_DIFFICULTY.value, GameState.BATTLE_START_PRACTICE.value):
            ai_context = {}
        ...
        if confirmed_state == GameState.BATTLE_ROOM.value and self._try_action('battle_room'):
            from config import TEMPLATE_ROI
        ...
        # 匹配成功 - 点击确认按钮
        logger.debug(f"状态检查: confirmed_state={confirmed_state}, MATCH_FOUND={GameState.MATCH_FOUND.value}")
        if confirmed_state in (GameState.MATCH_FOUND.value, GameState.MATCH_CONFIRMED.value):
            # 匹配确认按钮位置固定（基于confirm/hero_confirm模板实测: x=800,y=848,w=320,h=104）
            click_x, click_y = 960, 900
            logger.debug(f"匹配成功状态，模板检测结果: found=True, conf=1.000 (使用固定ROI坐标)")
            try:
                # 使用带验证的点击方法
                success = self._click_match_confirm_with_verification(click_x, click_y, img_gray)
                if success:
        ...
        if confirmed_state == GameState.GAME_END.value:
            ...
            buttons_to_try = []
            if detected_template and detected_template in ['victory', 'victory1', 'defeat']:
            ...
            for btn_template in buttons_to_try:
                ...
                if roi:
                    ...
                    click_y = roi['y'] + roi['h'] // 2
                    try:
                        logger.info(f"结算界面，直接点击{btn_template}: ({click_x}, {click_y})")
                        success = self.click_executor.click(click_x, click_y)
                        if success:
        ...
        if confirmed_state == GameState.MVP_DISPLAY.value:
            ...
            if roi:
                ...
                click_y = roi['y'] + roi['h'] // 2
                try:
                    logger.info(f"MVP展示状态，直接点击继续按钮: ({click_x}, {click_y})")
                    success = self.click_executor.click(click_x, click_y)
                    if success:
        ...
        if confirmed_state == GameState.POST_MATCH_STATS.value:
            ...
            if roi:
                ...
                click_y = roi['y'] + roi['h'] // 2
                try:
                    logger.info(f"赛后数据界面，直接点击返回房间: ({click_x}, {click_y})")
                    success = self.click_executor.click(click_x, click_y)
                    if success:
    ...
    def _detect_with_strategy(self, img_gray) -> Tuple[str, float, DetectionLevel]:
        ...
        if state:
            ...
            return state, detection_context.get('parent_confidence', 0.75), DetectionLevel.NORMAL
    ...
    def _confirm_state(self, detected_state: str, confidence: float, strategy: DetectionLevel = DetectionLevel.NORMAL) -> Tuple[str, bool]:
        """
        状态确认（连续帧防抖）
        ...
        # 根据状态确定确认阈值
        confirm_threshold = FAST_CONFIRM_STATES.get(detected_state, self.confirm_threshold)
        ...
        if detected_state in IMMEDIATE_CONFIRM_STATES and strategy != DetectionLevel.DEEP:
    ...
    def _transition_state(self, new_state: str, confidence: float, strategy: DetectionLevel):
        """
        执行状态切换
        ...
        if progress.percentage > 0:
            logger.info(f"  流程进度: {progress.percentage:.1f}% ({progress.current_index}/{progress.total_states})")
