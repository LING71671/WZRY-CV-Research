...
from utils.logging_utils import get_logger  # 导入日志工具

logger = get_logger(__name__)

try:
    from config import GAME_MODE, BATTLE_MODE, V5_MODE, AI_BATTLE_MODE, AI_DIFFICULTY, TEMPLATE_ROI  # 导入配置
except ImportError:
    # 提供默认值
...
class AIModeHandler:
    ...
    def __init__(self, click_executor, template_matcher):
        ...
        self.target_game_mode = self.GAME_MODE_MAP.get(GAME_MODE, '对战')  # 目标游戏模式
        self.target_battle_mode = self.BATTLE_MODE_MAP.get(BATTLE_MODE, '未知')  # 目标地图模式
        self.target_v5_mode = self.V5_MODE_MAP.get(V5_MODE, '未知')  # 目标匹配类型
        self.target_mode = self.MODE_MAP.get(AI_BATTLE_MODE)  # 目标游戏模式（标准/快速）
        ...
        # 显示完整目标配置
        logger.info(f"目标配置: 模式={self.target_game_mode}, 地图={self.target_battle_mode}, "
    ...
    def check_and_adjust(self, context: Dict[str, Any], img_gray) -> bool:
        ...
        if current_time - self.last_check_time < self.check_interval:  # 如果距离上次检查时间小于间隔
            return self.mode_correct and self.difficulty_correct  # 直接返回当前状态，不重复检查
        self.last_check_time = current_time  # 更新上次检查时间
        
        # 获取当前选中的模式和难度
        current_mode = context.get('ai_selected_mode')  # 从上下文中获取当前模式
        current_difficulty = context.get('ai_selected_difficulty')  # 从上下文中获取当前难度
        actual_difficulty = context.get('ai_actual_difficulty')  # 获取实际难度（备用）
        
        logger.debug(f"检查配置: 当前模式={current_mode}, 目标模式={self.target_mode}, "
        ...
        if current_mode == self.target_mode:  # 如果当前模式与目标匹配
            ...
            logger.info(f"模式不匹配: 当前={current_mode}, 目标={self.target_mode}, 准备点击切换")
        ...
        if AI_DIFFICULTY == 'recommend':  # 如果配置为推荐难度
            if current_difficulty == '推荐':  # 如果当前选中的是推荐
                ...
                logger.info(f"难度不匹配: 当前={current_difficulty}, 目标=推荐, 准备点击切换")
            ...
            if current_difficulty == self.target_difficulty:  # 如果当前难度与目标匹配
                ...
                self.difficulty_correct = True  # 设置难度正确标志
            elif current_difficulty == '推荐' and self.target_difficulty in all_difficulties:
                # 推荐难度的双重高亮处理：当前显示"推荐"，但实际难度与目标匹配
                logger.info(f"推荐难度实际为{self.target_difficulty}，配置匹配")
                self.difficulty_correct = True  # 设置难度正确标志
            else:  # 如果难度不匹配
                logger.info(f"难度不匹配: 当前={current_difficulty}, 目标={self.target_difficulty}, 准备点击切换")
                self.difficulty_correct = False  # 设置难度不正确标志
                self._click_difficulty(self.target_difficulty, img_gray)  # 点击切换到目标难度
                return False  # 返回失败
    ...
    def _click_mode(self, mode_name: str, img_gray):
        ...
        if result.found:  # 如果模板匹配成功
            ...
            logger.info(f"点击切换模式: {mode_name} (模板匹配位置: {click_x}, {click_y})")
            ...
            if template_name in TEMPLATE_ROI:  # 如果ROI配置中存在该模板
                ...
                logger.info(f"点击切换模式: {mode_name} (ROI位置: {click_x}, {click_y})")
                self.click_executor.click(click_x, click_y)  # 执行点击
            else:  # 如果ROI配置中也不存在
    ...
    def _click_difficulty(self, difficulty_name: str, img_gray):
        ...
        if result.found:  # 如果模板匹配成功
            ...
            click_x = x + w // 2  # 计算中心点X坐标
            click_y = y + h // 2  # 计算中心点Y坐标
            logger.info(f"点击切换难度: {difficulty_name} (模板匹配位置: {click_x}, {click_y})")
            self.click_executor.click(click_x, click_y)  # 执行点击
        else:  # 如果模板匹配失败
            # 模板匹配失败，使用ROI配置中的位置
            if template_name in TEMPLATE_ROI:  # 如果ROI配置中存在该模板
                ...
                click_x = roi['x'] + roi['w'] // 2  # 计算中心点X坐标
                click_y = roi['y'] + roi['h'] // 2  # 计算中心点Y坐标
                logger.info(f"点击切换难度: {difficulty_name} (ROI位置: {click_x}, {click_y})")
                self.click_executor.click(click_x, click_y)  # 执行点击
            else:  # 如果ROI配置中也不存在
