...
ADB_DEVICE_SERIAL = f"{ADB_DEVICE_IP}:{ADB_DEVICE_PORT}"  # 完整设备序列号
...
MODEL1_WEIGHTS = os.fspath(
    _PATH_RESOLVER.resolve_model("best_perfect.pt")
)  # 模型1权重文件（英雄检测）
MODEL2_WEIGHTS = os.fspath(
    _PATH_RESOLVER.resolve_model("WZRY-health.pt")
)  # 模型2权重文件（血条检测）
MODEL3_WEIGHTS = os.fspath(
    _PATH_RESOLVER.resolve_model("wzry.pt")
